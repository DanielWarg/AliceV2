...
def test_planner_v2_direct():
    ...
    for i, scenario in enumerate(TEST_SCENARIOS, 1):
        ...
        print(
            f"🎯 Expected: intent={scenario['expected_intent']}, tool={scenario['expected_tool']}"
        )
        ...
        try:
            ...
            try:
                ...
                render_instruction = parsed_response.get(
                    "render_instruction", "unknown"
                )
                ...
                assert render_instruction in [
                    "chart",
                    "map",
                    "scene",
                    "none",
                ], f"Invalid render_instruction: {render_instruction}"
                ...
                assert (
                    meta.get("version") == "4.0"
                ), f"Invalid version: {meta.get('version')}"
                assert (
                    meta.get("schema_version") == "v4"
                ), f"Invalid schema_version: {meta.get('schema_version')}"
                ...
                print(
                    f"📊 Result: intent='{intent}' (match={intent_match}), tool='{tool}' (match={tool_match})"
                )
            ...
            results.append(
                {
                    ...
                    "tool": tool,
                    "intent_match": (
                        intent_match if "intent_match" in locals() else False
                    ),
    ...
    print(
        f"📋 Schema OK Rate: {schema_ok_tests}/{total_tests} ({schema_ok_tests/total_tests*100:.1f}%)"
    )
    print(
        f"🔍 JSON Parsed Rate: {json_parsed_tests}/{total_tests} ({json_parsed_tests/total_tests*100:.1f}%)"
    )
    print(
        f"🎯 Intent Match Rate: {intent_match_tests}/{total_tests} ({intent_match_tests/total_tests*100:.1f}%)"
    )
    print(
        f"🔧 Tool Match Rate: {tool_match_tests}/{total_tests} ({tool_match_tests/total_tests*100:.1f}%)"
    )
    print(
        f"📊 MEDIUM Success: {medium_success}/{len(medium_tests)} ({medium_success/len(medium_tests)*100:.1f}%)"
    )
    print(
        f"📊 HARD Success: {hard_success}/{len(hard_tests)} ({hard_success/len(hard_tests)*100:.1f}%)"
    )
    ...
    print(
        f"📋 Schema OK @ first ≥ 98% (EASY+MEDIUM): {schema_ok_rate:.1%} {'✅' if schema_ok_pass else '❌'}"
    )
    ...
    print(
        f"🎯 Intent match ≥ 95%: {intent_match_rate:.1%} {'✅' if intent_match_pass else '❌'}"
    )
