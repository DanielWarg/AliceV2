...
async def warm_cache(base_url: str = "http://localhost:18000", regression_dir: str = "eval/regression"):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, response in enumerate(responses):
            if isinstance(response, Exception):
                print(f"❌ Failed to warm cache for scenario {easy_medium_scenarios[i]['id']}: {response}")
            else:
                successful += 1
                if i < 5:  # Show first 5 responses
                    print(f"✅ Warmed cache: {easy_medium_scenarios[i]['prompt_sv'][:30]}...")
    
    print(f"🎉 Cache warming complete: {successful}/{len(easy_medium_scenarios)} scenarios cached")
...
def main():
    ...
    parser.add_argument("--check-only", action="store_true", help="Only check cache status, don't warm")
