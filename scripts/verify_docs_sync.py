...
class DocSyncVerifier:
    ...
    def check_port_consistency(self):
        ...
        docs_to_check = ["README.md", "ALICE_PRODUCTION_CHECKLIST.md", "docker-compose.yml", "ALICE_SYSTEM_BLUEPRINT.md"]
        ...
        for doc, ports in port_refs.items():
            for port in ports:
                ...
                elif port not in ["3000", "8000", "8001", "8002", "8003", "8004", "8005"]:  # Old ports we're migrating from
    ...
    def check_system_status_consistency(self):
        ...
        if missing_from_checklist:
            for system in missing_from_checklist:
                self.issues.append(f"❌ ALICE_PRODUCTION_CHECKLIST.md missing system: {system}")
    ...
    def check_docker_services_match(self):
        ...
        if missing_docs:
            for service in missing_docs:
                self.issues.append(f"❌ Service '{service}' in docker-compose.yml but not documented in blueprint")
                
        if extra_docs:
            for service in extra_docs:
                self.issues.append(f"❌ Service '{service}' documented in blueprint but not in docker-compose.yml")
