...
class MarkdownSyncSystem:
    ...
    def _extract_references(self, content: str) -> List[str]:
        ...
        phase_refs = re.findall(
            r"Phase \d+|v\d+\.\d+|version \d+", content, re.IGNORECASE
        )
    ...
    def _extract_status_indicators(self, content: str) -> List[str]:
        ...
        # Version indicators
        versions = re.findall(
            r"v\d+\.\d+\.\d+|version \d+\.\d+", content, re.IGNORECASE
        )
    ...
    def detect_duplicates(self, all_analyses: List[Dict]) -> Dict[str, List[str]]:
        ...
        for analysis in all_analyses:
            if "headers" in analysis and analysis["headers"]:
                main_header = (
                    analysis["headers"][0] if analysis["headers"] else "no_header"
                )
    ...
    def generate_sync_recommendations(self, all_analyses: List[Dict]) -> Dict:
        ...
        for analysis in all_analyses:
            ...
            if (
                "test_results" in file_path
                or "DAILY_STATUS" in file_path
                or analysis.get("size", 0) < 100
            ):  # Very small files
                recommendations["archive_candidates"].append(file_path)
        ...
        for group_name, files in duplicates.items():
            if "similar" in group_name and len(files) > 1:
                recommendations["merge_candidates"].append(
                    {"group": group_name, "files": files, "action": "consider_merging"}
                )
...
def main():
    ...
    print(
        f"Critical updates needed: {len(sync_plan['recommendations']['critical_updates'])}"
    )
    print(
        f"Archive candidates: {len(sync_plan['recommendations']['archive_candidates'])}"
    )
