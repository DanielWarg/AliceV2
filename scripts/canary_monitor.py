...
class CanaryMonitor:
    ...
    def update_gates(self, stats: Dict):
        ...
        self.gates["tool_choice_same"]["current"] = stats.get(
            "tool_choice_same_rate", 0.0
        )
        self.gates["latency_delta_p95"]["current"] = abs(
            stats.get("avg_latency_delta_ms", 0.0)
        )
        self.gates["canary_routed_rate"]["current"] = stats.get(
            "canary_routed", 0
        ...
        for gate_name, gate in self.gates.items():
            if "min" in gate:
                ...
                gate["status"] = (
                    "green" if abs(current - target) <= tolerance else "yellow"
                )
    ...
    def check_escalation(self) -> Optional[float]:
        ...
        for escalation in self.escalation_schedule:
            if (
                elapsed_hours >= escalation["time_hours"]
                and self.current_canary_percent < escalation["target_percent"]
            ):
                # Check if all gates are green
                all_green = all(
                    gate["status"] == "green" for gate in self.gates.values()
                )
                if all_green:
    ...
    def print_status(self):
        ...
        for gate_name, gate in self.gates.items():
            status_emoji = (
                ...
                else "🔴" if gate["status"] == "red" else "🟡"
            )
            current = gate["current"]

            if "min" in gate:
                print(
                    f"{status_emoji} {gate_name}: {current:.3f} (min: {gate['min']:.3f})"
                )
            elif "max" in gate:
                print(
                    f"{status_emoji} {gate_name}: {current:.1f}ms (max: {gate['max']:.1f}ms)"
                )
            elif "target" in gate:
                print(
                    f"{status_emoji} {gate_name}: {current:.1%} (target: {gate['target']:.1%})"
                )
    ...
    async def run_monitor(self):
        ...
        while True:
            try:
                ...
                if rollback_reason:
                    await self.update_canary_percent(
                        0.0, f"auto_rollback_{rollback_reason}"
                    )
