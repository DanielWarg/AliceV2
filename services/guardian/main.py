...
def update_state():
    ...
    hard = (
        ram >= RAM_HARD
        or (temp and temp >= TEMP_HARD)
        or (batt is not None and batt <= BATT_HARD)
    )

    prev = S.state
    if prev in ("NORMAL", "DEGRADED") and hard:
        S.state = "EMERGENCY"
        S.brownout_level = "HEAVY"
        S.reason = "HARD_TRIGGER"
        LAST_TS = time.time()
    elif prev == "NORMAL" and soft:
        # börja lätt brownout
        S.state = "BROWNOUT"
        S.brownout_level = "LIGHT"
        S.reason = "SOFT_TRIGGER"
        LAST_TS = time.time()
