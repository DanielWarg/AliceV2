...
from .brownout_manager import BrownoutConfig, BrownoutLevel, BrownoutManager
from .guardian_state import GuardianConfig, GuardianState
...
class Guardian:
    ...
    def __init__(self, config: GuardianConfig):
        ...
        self.lockdown_until: datetime | None = None
    ...
    async def _evaluate_system_state(self, metrics: SystemMetrics) -> GuardianState:
        ...
        if (
            metrics.ram_pct >= self.config.ram_hard_pct * 100
            or metrics.cpu_pct >= self.config.cpu_hard_pct * 100
            or metrics.disk_pct >= self.config.disk_hard_pct * 100
            or (metrics.temp_c and metrics.temp_c >= self.config.temp_hard_c)
        ):
            return GuardianState.EMERGENCY

        # Soft triggers (with hysteresis)
        is_soft_trigger = (
            metrics.ram_pct >= self.config.ram_soft_pct * 100
            or metrics.cpu_pct >= self.config.cpu_soft_pct * 100
        )
        ...
        if len(self.soft_trigger_measurements) == self.config.measurement_window:
            if all(self.soft_trigger_measurements):
                ...
                if self.state == GuardianState.BROWNOUT:
                    return GuardianState.DEGRADED

        # Check for recovery
        is_recovery = (
            metrics.ram_pct <= self.config.ram_recovery_pct * 100
            and metrics.cpu_pct <= self.config.cpu_recovery_pct * 100
        )
        ...
        if is_recovery:
            if self.recovery_start_time is None:
                ...
                datetime.now() - self.recovery_start_time
            ).total_seconds() >= self.config.recovery_window_s:
    ...
    async def _transition_to_state(
        self, new_state: GuardianState, metrics: SystemMetrics
    ):
    ...
    async def _execute_emergency_procedure(self, metrics: SystemMetrics):
        ...
        self.logger.warning(
            f"EMERGENCY: RAM={metrics.ram_pct:.1f}% CPU={metrics.cpu_pct:.1f}%"
        )
        ...
        if not self._can_execute_kill():
            ...
            self.lockdown_until = datetime.now() + timedelta(
                seconds=self.config.lockdown_duration_s,
            )
            return
    ...
    def _update_state_duration(self):
        ...
        if hasattr(self.metrics_history, "__iter__") and self.metrics_history:
            ...
            latest_metrics.degraded = self.state in [
                GuardianState.DEGRADED,
                GuardianState.EMERGENCY,
                GuardianState.LOCKDOWN,
            ]
            latest_metrics.emergency_mode = self.state in [
                GuardianState.EMERGENCY,
                GuardianState.LOCKDOWN,
            ]
            latest_metrics.intake_blocked = self.state in [
                GuardianState.EMERGENCY,
                GuardianState.LOCKDOWN,
            ]
    ...
    def _log_metrics(self, metrics: SystemMetrics):
        ...
        log_data = {
            ...
            "state_duration_s": (
                datetime.now() - self.state_start_time
            ).total_seconds(),
            ...
            "brownout_level": (
                self.brownout_manager.state.level.name
                if self.brownout_manager.state.active
                else None
            ),
        }
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        status = {
            ...
            "metrics": {
                ...
                "intake_blocked": self.state
                in [GuardianState.EMERGENCY, GuardianState.LOCKDOWN],
                "emergency_mode": self.state
                in [GuardianState.EMERGENCY, GuardianState.LOCKDOWN],
            },
        }
