...
class GracefulKillSequence:
    ...
    async def execute_kill_sequence(self) -> bool:
        ...
        try:
            ...
            if not await self._resume_intake():
                self.logger.warning("Failed to resume intake, manual intervention may be needed")
    ...
    async def _stop_intake_and_drain(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Draining request queue for {self.config.drain_timeout_s}s")
    ...
    async def _graceful_terminate_ollama(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Waiting {self.config.sigterm_timeout_s}s for graceful shutdown")
            ...
            if remaining_pids:
                self.logger.warning(f"Force killing remaining processes: {remaining_pids}")
    ...
    async def _restart_ollama_with_backoff(self) -> bool:
        ...
        max_attempts = min(self.config.max_restart_attempts, len(self.config.restart_delays))
        
        for attempt in range(max_attempts):
            delay = self.config.restart_delays[min(attempt, len(self.config.restart_delays) - 1)]
            
            self.logger.info(f"Restart attempt {attempt + 1}/{max_attempts} after {delay}s delay")
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            "loaded": True,
            "last_execution": self.last_kill_time.isoformat() if self.last_kill_time else None,
