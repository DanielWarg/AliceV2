...
class BrownoutConfig:
    ...
    def __post_init__(self):
        if self.tools_to_disable is None:
            self.tools_to_disable = ["code_interpreter", "file_search", "web_search"]
...
class BrownoutManager:
    ...
    async def activate_brownout(self, level: BrownoutLevel) -> bool:
        ...
        try:
            ...
            if level >= BrownoutLevel.MODERATE:
                # Context reduction
                if not await self._reduce_context_window(
                    self.config.context_window_reduced
                ):
                    success = False
            ...
            if success:
                ...
                self.logger.error(
                    f"Failed to fully activate brownout level {level.name}"
                )
    ...
    async def deactivate_brownout(self) -> bool:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    f"Brownout deactivated successfully after {final_duration:.1f}s"
                )
            else:
    ...
    def get_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "activation_time": (
                self.state.activation_time.isoformat()
