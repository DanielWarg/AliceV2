...
def execute_stress_scenario(scenarios: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if step_kind == "deep_bomb":
                ...
                result = memory_balloon(
                    gb=step.get("gb", 4), hold_seconds=step.get("hold_seconds", 60)
                )
            elif step_kind == "cpu_spin":
                result = cpu_spin(
                    threads=step.get("threads", 4), seconds=step.get("seconds", 30)
                )
            elif step_kind == "tool_storm":
                result = tool_storm(
                    rps=step.get("rps", 2), seconds=step.get("seconds", 20)
                )
            elif step_kind == "vision_stress":
                result = vision_stress(seconds=step.get("seconds", 30))
            else:
    ...
    results["_meta"] = {
        ...
        "completed_steps": len(
            [r for r in results.values() if not isinstance(r, dict) or "error" not in r]
        ),
        "failed_steps": len(
...
def main():
    ...
    try:
        ...
        write_guardian_event(
            {"event": "baseline", "guardian_state": baseline_state}, RUN_DIR
        )
        ...
        if stress_thread.is_alive():
            print("⚠️  Stress test still running after timeout")
        ...
        summary = {
            ...
            "environment": {
                ...
                "guard_health": os.getenv(
                    "GUARD_HEALTH", "http://localhost:8787/health"
                ),
        ...
        if trigger_latency_ms is not None:
            ...
            print(
                f"Brownout Trigger: {trigger_latency_ms}ms (budget: {budget_ms}ms) {trigger_status}"
            )
        else:
            print("Brownout Trigger: ❌ FAIL (No brownout detected)")
        ...
        if recovery_time_s is not None:
            ...
            print(
                f"Recovery Time: {recovery_time_s}s (budget: {budget_s}s) {recovery_status}"
            )
        else:
            print("Recovery Time: ❌ FAIL (No recovery detected)")

        overall_status = "✅ PASS" if slo_pass else "❌ FAIL"
        print(f"Overall SLO: {overall_status}")

        print(f"\n📁 Results saved to: {RUN_DIR}")
        print("   summary.json - Complete test results")
        print("   slo_result.json - SLO compliance details")
        print("   result.txt - Simple PASS/FAIL")
        print("   guardian_events.jsonl - Guardian state transitions")
