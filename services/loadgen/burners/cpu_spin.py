...
def run(threads: int = None, seconds: int = 30) -> Dict[str, Any]:
    ...
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            while time.perf_counter() < end_time:
                ...
                futures = [
                    executor.submit(_cpu_burn_cycle, 5_000_000)  # Smaller cycles for responsiveness
                ...
                if len(cpu_samples) == 0 or time.perf_counter() - cpu_samples[-1]['time'] > 2:
        ...
        print(f"   CPU stress test interrupted")
    ...
    avg_cpu = sum(s['cpu_percent'] for s in cpu_samples) / len(cpu_samples) if cpu_samples else 0
    max_cpu = max(s['cpu_percent'] for s in cpu_samples) if cpu_samples else 0
    avg_load = sum(s['load_1min'] for s in cpu_samples) / len(cpu_samples) if cpu_samples else 0
    ...
    print(f"   Completed: {total_cycles} cycles, avg CPU: {avg_cpu:.1f}%, "
          f"max: {max_cpu:.1f}%")
