...
def send_vision_request(client: httpx.Client, request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        vision_processed = any(keyword in response_text.lower() 
...
def run(seconds: int = 30) -> Dict[str, Any]:
    ...
    if CAMERA_RTSP_URL:
        ...
        print(f"   Note: No RTSP URL configured - using mock responses")
    ...
    with httpx.Client(timeout=20.0) as client:
        while time.perf_counter() < end_time:
            ...
            if request_counter % 3 == 0 or elapsed >= seconds:
                print(f"   Sent {request_counter} vision requests "
    ...
    result_summary = {
        ...
        "vision_processed": vision_processed,
        "vision_processing_rate": vision_processed / total_requests if total_requests > 0 else 0,
    ...
    if vision_processed > 0:
        print(f"   Vision processed: {vision_processed}/{total_requests} "
