...
def run(gb: float = None, hold_seconds: int = 60) -> Dict[str, Any]:
    ...
    print(
        f"   Baseline RAM: {baseline_stats['used_gb']:.1f}GB "
    ...
    try:
        ...
        while allocated_bytes < target_bytes:
            try:
                ...
                print(
                    f"   Allocated: {allocated_bytes/(1024**3):.1f}GB "
                    f"({progress_pct:.1f}%), RAM: {current_stats['percent']:.1f}%"
                )

                # Check if we've hit Guardian thresholds
                if current_stats["percent"] > 90:
                    print("   🟡 RAM > 90% - Guardian should trigger brownout soon")
                elif current_stats["percent"] > 85:
                    print("   ⚠️  RAM > 85% - Approaching brownout threshold")

                time.sleep(ALLOCATION_DELAY_S)

            except MemoryError:
                print(
                    f"   ❌ Memory allocation failed at {allocated_bytes/(1024**3):.1f}GB"
                )
                break
        ...
        while time.perf_counter() - hold_start < hold_seconds:
            ...
            if int(time.perf_counter() - hold_start) % 15 == 0:  # Every 15s
                ...
                print(
                    f"   Holding... RAM: {current_stats['percent']:.1f}%, "
    ...
    print(
        f"   Completed: {allocated_bytes/(1024**3):.1f}GB allocated, "
