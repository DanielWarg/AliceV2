...
def create_timeline_chart(events: List[Dict[str, Any]], summary: Dict[str, Any]) -> go.Figure:
    """Create timeline chart showing Guardian state transitions"""
    
    fig = go.Figure()
    
    if not events:
        fig.add_annotation(text="No guardian events found", 
    ...
    if not timeline_data:
        fig.add_annotation(text="No valid timeline data", 
                          xref="paper", yref="paper", 
                          x=0.5, y=0.5, showarrow=False)
        return fig
...
def create_metrics_summary(summary: Dict[str, Any], result: str) -> None:
    ...
    with col3:
        # Overall SLO status
        overall_pass = (
            (trigger_ms is not None and trigger_ms <= SLO_BROWNOUT_TRIGGER_MS) and
            (recovered_s is not None and recovered_s <= SLO_RECOVERY_S)
        )
    ...
    with col4:
        # Test components
        stress_tests = len([k for k in summary.keys() 
