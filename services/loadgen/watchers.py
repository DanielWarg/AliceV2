...
class GuardianWatcher:
        ...
        try:
            ...
            if current_state == "EMERGENCY":
                if self.emergency_start is None:
                    ...
                    raise RuntimeError(f"EMERGENCY state exceeded {EMERGENCY_TIMEOUT_S}s - aborting for safety")
            else:
                self.emergency_start = None
    ...
    def wait_for_brownout(self, start_ts: Optional[float] = None) -> Tuple[Optional[int], Dict[str, Any]]:
        """
        ...
        while True:
            ...
            if current_state in ["BROWNOUT", "EMERGENCY"]:
                ...
                print(f"🟡 Brownout triggered: {current_state} after {trigger_latency}ms")
                if brownout_level:
            ...
            if time.perf_counter() - t0 > 120:  # 2 minutes max wait
                print(f"⚠️  No brownout after 120s - test may have failed")
                return None, data
    ...
    def wait_for_recovery(self) -> Optional[int]:
        ...
        while True:
            ...
            if current_state == "NORMAL" and ram_pct < 75:
                ...
                print(f"🟢 Recovery complete: NORMAL state, RAM {ram_pct:.1f}% after {recovery_time}s")
            ...
            if elapsed % 10 == 0:  # Progress update every 10s
                print(f"   Still recovering... {current_state}, RAM {ram_pct:.1f}% ({elapsed:.0f}s)")
...
def write_slo_result(run_dir: str, trigger_ms: Optional[int], recovery_s: Optional[int]) -> bool:
