...
class IntentValidator:
    def __init__(self, registry, enabled: bool = True):
        ...
        self._init_xnli()
        print(f"XNLI INIT DONE: sess={self._sess is not None}, tokenizer={self._tokenizer}")
    ...
    def _entailment_prob(self, premise: str, hypothesis: str) -> Optional[float]:
        ...
        if self._tokenizer == "keyword_fallback":
            ...
            weather_keywords = ["väder", "weather", "temperatur", "temperature", "prognos", "forecast", "vädret", "vad är vädret"]
            time_keywords = ["klockan", "tid", "datum", "time", "date", "när", "when", "hur mycket är klockan"]
            calendar_keywords = ["boka", "skapa", "kalender", "calendar", "möte", "meeting", "event"]
            email_keywords = ["mail", "email", "e-post", "skicka", "send", "meddelande", "message"]
            memory_keywords = ["kom ihåg", "remember", "minne", "memory", "spara", "save", "glöm", "forget"]
    ...
    def validate(self, text: str, labels: list[str]) -> tuple[bool, str]:
        print(f"XNLI VALIDATE: enabled={self.enabled}, sess={self._sess is not None}, tokenizer={self._tokenizer is not None}, labels={labels}")
        ...
        print(f"XNLI DEBUG: text='{text}', best='{best_label}' (p1={p1:.3f}), second='{second_label}' (p2={p2:.3f}), thresh={self._ent_thresh}")
