...
class SLOMonitor:
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    metrics.avg_response_ms = data.get("metrics", {}).get("avg_response_ms", 0.0)
                    metrics.success_rate = data.get("metrics", {}).get("success_rate", 1.0)
                else:
                    self.logger.warning(f"Orchestrator health check failed: {response.status_code}")
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(f"Guardian health check failed: {response.status_code}")
            ...
            if trends_file.exists():
                ...
                with open(trends_file, 'r') as f:
                    ...
                    if lines:
                        ...
                        metrics.test_success_rate = 1.0 if last_trend.get("slo_compliant", True) else 0.8
    ...
    def calculate_weighted_health_score(self, metrics: SystemMetrics, violations: Dict[str, float]) -> int:
        """
        ...
        if metrics.error_429_rate > self.thresholds.error_429_yellow_rate:
            error_penalty += 10 if metrics.error_429_rate > self.thresholds.error_429_red_rate else 5
        if metrics.error_5xx_rate > self.thresholds.error_5xx_yellow_rate:
            error_penalty += 15 if metrics.error_5xx_rate > self.thresholds.error_5xx_red_rate else 8
        penalty += min(error_penalty, 25)  # Cap at 25 points
        
        # Guardian state penalties (20% weight)
        guardian_penalty = {
            "LOCKDOWN": 20, "EMERGENCY": 18, "DEGRADED": 15,
            "BROWNOUT": {"HEAVY": 12, "MODERATE": 8, "LIGHT": 5}.get(metrics.guardian_level, 8),
            "NORMAL": 0
    ...
    def evaluate_slo(self, metrics: SystemMetrics) -> SLOResult:
        ...
        if metrics.p95_fast_ms > self.thresholds.fast_p95_budget_ms * self.thresholds.fast_p95_red_pct:
            overrun = metrics.p95_fast_ms / self.thresholds.fast_p95_budget_ms
            violations['p95_fast'] = overrun
            issues.append(f"Critical: Fast route P95 {metrics.p95_fast_ms:.0f}ms > {self.thresholds.fast_p95_budget_ms * self.thresholds.fast_p95_red_pct:.0f}ms")
            recommendations.append("Scale fast route capacity or implement degrade & gate")
            status = SLOStatus.RED
        elif metrics.p95_fast_ms > self.thresholds.fast_p95_budget_ms * self.thresholds.fast_p95_yellow_pct:
            overrun = metrics.p95_fast_ms / self.thresholds.fast_p95_budget_ms  
            violations['p95_fast'] = overrun
            issues.append(f"Warning: Fast route P95 {metrics.p95_fast_ms:.0f}ms > {self.thresholds.fast_p95_budget_ms * self.thresholds.fast_p95_yellow_pct:.0f}ms")
        ...
        if metrics.p95_planner_full_ms > self.thresholds.planner_full_p95_budget_ms * self.thresholds.planner_red_pct:
            overrun = metrics.p95_planner_full_ms / self.thresholds.planner_full_p95_budget_ms
            violations['p95_planner'] = overrun
            issues.append(f"Critical: Planner P95 {metrics.p95_planner_full_ms:.0f}ms > {self.thresholds.planner_full_p95_budget_ms * self.thresholds.planner_red_pct:.0f}ms")
            recommendations.append("Reduce planner context size or enable brownout mode")
            status = SLOStatus.RED
        elif metrics.p95_planner_full_ms > self.thresholds.planner_full_p95_budget_ms * self.thresholds.planner_yellow_pct:
            overrun = metrics.p95_planner_full_ms / self.thresholds.planner_full_p95_budget_ms
            violations['p95_planner'] = overrun  
            issues.append(f"Warning: Planner P95 {metrics.p95_planner_full_ms:.0f}ms > {self.thresholds.planner_full_p95_budget_ms * self.thresholds.planner_yellow_pct:.0f}ms")
            recommendations.append("Consider reducing planner complexity")
            if status == SLOStatus.GREEN:
                status = SLOStatus.YELLOW
                
        # Deep route (≤1800/3000ms budget)
        if metrics.p95_deep_full_ms > self.thresholds.deep_full_p95_budget_ms * self.thresholds.deep_red_pct:
            overrun = metrics.p95_deep_full_ms / self.thresholds.deep_full_p95_budget_ms
            violations['p95_deep'] = overrun
            issues.append(f"Critical: Deep route P95 {metrics.p95_deep_full_ms:.0f}ms > {self.thresholds.deep_full_p95_budget_ms * self.thresholds.deep_red_pct:.0f}ms")
            recommendations.append("Emergency: Degrade deep processing or gate traffic")
            status = SLOStatus.RED
        elif metrics.p95_deep_full_ms > self.thresholds.deep_full_p95_budget_ms * self.thresholds.deep_yellow_pct:
            overrun = metrics.p95_deep_full_ms / self.thresholds.deep_full_p95_budget_ms
            violations['p95_deep'] = overrun
            issues.append(f"Warning: Deep route P95 {metrics.p95_deep_full_ms:.0f}ms > {self.thresholds.deep_full_p95_budget_ms * self.thresholds.deep_yellow_pct:.0f}ms")
        ...
        if metrics.error_429_rate > self.thresholds.error_429_red_rate:
            issues.append(f"Critical: 429 rate {metrics.error_429_rate:.1%} > {self.thresholds.error_429_red_rate:.1%} (admission control failure)")
            recommendations.append("Emergency: Implement aggressive admission control and degrade & gate")
            status = SLOStatus.RED
        elif metrics.error_429_rate > self.thresholds.error_429_yellow_rate:
            issues.append(f"Warning: 429 rate {metrics.error_429_rate:.1%} > {self.thresholds.error_429_yellow_rate:.1%} (high traffic)")
            recommendations.append("Consider preemptive brownout mode")
            if status == SLOStatus.GREEN:
                status = SLOStatus.YELLOW
                
        if metrics.error_5xx_rate > self.thresholds.error_5xx_red_rate:
            issues.append(f"Critical: 5xx rate {metrics.error_5xx_rate:.1%} > {self.thresholds.error_5xx_red_rate:.1%} (server failures)")
            recommendations.append("Emergency: Investigate service failures and restart components")
            status = SLOStatus.RED
        elif metrics.error_5xx_rate > self.thresholds.error_5xx_yellow_rate:
            issues.append(f"Warning: 5xx rate {metrics.error_5xx_rate:.1%} > {self.thresholds.error_5xx_yellow_rate:.1%} (service degradation)")  
        ...
        if metrics.ram_pct > self.thresholds.ram_hard_pct:
            issues.append(f"Critical: RAM usage {metrics.ram_pct:.1%} > {self.thresholds.ram_hard_pct:.1%} (hard trigger)")
            recommendations.append("Emergency: Trigger Guardian emergency mode immediately")
            status = SLOStatus.RED
        elif metrics.ram_pct > self.thresholds.ram_soft_pct:
            issues.append(f"Warning: RAM usage {metrics.ram_pct:.1%} > {self.thresholds.ram_soft_pct:.1%} (soft trigger)")
            recommendations.append("Trigger Guardian brownout mode and degrade & gate")
            if status == SLOStatus.GREEN:
                status = SLOStatus.YELLOW
            
        if metrics.cpu_pct > self.thresholds.cpu_hard_pct:
            issues.append(f"Critical: CPU usage {metrics.cpu_pct:.1%} > {self.thresholds.cpu_hard_pct:.1%} (hard trigger)")
            recommendations.append("Emergency: Reduce CPU load or scale resources")
            status = SLOStatus.RED
        elif metrics.cpu_pct > self.thresholds.cpu_soft_pct:
            issues.append(f"Warning: CPU usage {metrics.cpu_pct:.1%} > {self.thresholds.cpu_soft_pct:.1%} (soft trigger)")
        ...
        if metrics.temp_c and metrics.temp_c >= self.thresholds.temp_red_c:
            issues.append(f"Critical: Temperature {metrics.temp_c:.1f}°C ≥ {self.thresholds.temp_red_c}°C")
            recommendations.append("Emergency: Reduce processing load to prevent thermal throttling")
            status = SLOStatus.RED
        elif metrics.temp_c and metrics.temp_c >= self.thresholds.temp_yellow_c:
            issues.append(f"Warning: Temperature {metrics.temp_c:.1f}°C ≥ {self.thresholds.temp_yellow_c}°C")
            recommendations.append("Monitor thermal state and consider cooling")
            if status == SLOStatus.GREEN:
                status = SLOStatus.YELLOW
                
        if metrics.battery_pct and metrics.battery_pct <= self.thresholds.battery_red_pct:
            issues.append(f"Critical: Battery {metrics.battery_pct:.1f}% ≤ {self.thresholds.battery_red_pct}%")
            recommendations.append("Emergency: Enable power saving mode and reduce load")
            status = SLOStatus.RED  
        elif metrics.battery_pct and metrics.battery_pct <= self.thresholds.battery_yellow_pct:
            issues.append(f"Warning: Battery {metrics.battery_pct:.1f}% ≤ {self.thresholds.battery_yellow_pct}%")
        ...
        if not metrics.guardian_available:
            ...
            if metrics.brownout_reason:
                issues.append(f"Trigger: {metrics.brownout_reason} ({metrics.brownout_since_s:.1f}s ago)")
            recommendations.append("Investigate system overload and degrade & gate traffic")
            status = SLOStatus.RED
        elif metrics.guardian_state == "DEGRADED":
            issues.append(f"Critical: Guardian in DEGRADED state")
            recommendations.append("System in recovery mode - monitor closely")
            status = SLOStatus.RED  
        elif metrics.guardian_state == "BROWNOUT":
            level_desc = f" ({metrics.guardian_level})" if metrics.guardian_level else ""
            issues.append(f"Warning: Guardian in BROWNOUT{level_desc} mode")
            if metrics.brownout_since_s > 300:  # > 5 minutes
                recommendations.append(f"Brownout active for {metrics.brownout_since_s/60:.1f}min - investigate load reduction")
            else:
        ...
        if metrics.test_success_rate < 0.85:  # 85% minimum for tests
            issues.append(f"Warning: Test success rate {metrics.test_success_rate:.1%} below 85%")
    ...
    async def check_and_alert(self) -> SLOResult:
        ...
        if result.status != self.current_status:
            self.logger.info(f"SLO status changed: {self.current_status} -> {result.status}")
