...
class PlannerOutput(BaseModel):
    ...
    version: Optional[int] = (
        1  # Make version optional since model doesn't always include it
    )
...
class ToolStep(BaseModel):
    ...
    timeout_ms: Optional[int] = Field(
        3000, description="Timeout for this tool call in milliseconds"
    )
...
class Plan(BaseModel):
    ...
    steps: List[ToolStep] = Field(
        default_factory=list, description="List of tool steps to execute"
    )
    response: str = Field(..., description="Response to give to the user")
    guardrails: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Safety guardrails"
    )
