...
import os
import redis
from ..cache_key import canonical_prompt, build_cache_key, micro_key
...
class CacheResult:
    ...
    def __init__(self, hit: bool, data: Any = None, reason: str = "", 
                 source: str = "miss", latency_ms: float = 0.0):
        self.hit = hit
        self.data = data
        self.reason = reason
...
class SmartCache:
    ...
    async def get(self, intent: str, prompt: str, model_id: str = "qwen2.5:3b", 
                  schema_version: str = "v4") -> CacheResult:
        ...
        try:
            ...
            if l1_result:
                ...
                return CacheResult(True, json.loads(l1_result), "l1_exact_match", "l1", latency_ms)
            ...
            if self.redis_client.get(neg_key):
                ...
                return CacheResult(True, self._get_negative_response(intent), "negative_cache", "negative", latency_ms)
    ...
    async def _semantic_lookup(self, intent: str, prompt: str, model_id: str) -> CacheResult:
        ...
        try:
            ...
            if best_data:
                logger.info("L2 semantic HIT", 
                           similarity=best_similarity,
                           threshold=self.semantic_threshold,
                           key=best_key[-20:])
                
                return CacheResult(True, best_data, f"semantic_match_{best_similarity:.2f}", "l2")
    ...
    def set(self, intent: str, prompt: str, response: Dict[str, Any], 
            model_id: str = "qwen2.5:3b", schema_version: str = "v4", ttl: int = 300):
    ...
    def set_negative(self, prompt: str, intent: str = "unknown", ttl: int = 60):
        ...
        try:
            ...
            logger.info("Negative cache SET", prompt=prompt[:30], intent=intent, ttl=ttl)
    ...
    def _update_hit_latency(self, latency_ms: float):
        ...
        total_hits = self.stats["l1_hits"] + self.stats["l2_hits"] + self.stats["negative_hits"]
        if total_hits > 1:
            self.stats["avg_hit_latency_ms"] = (
                (self.stats["avg_hit_latency_ms"] * (total_hits - 1) + latency_ms) / total_hits
            )
        else:
    ...
    def _update_miss_latency(self, latency_ms: float):
        ...
        if self.stats["misses"] > 1:
            self.stats["avg_miss_latency_ms"] = (
                (self.stats["avg_miss_latency_ms"] * (self.stats["misses"] - 1) + latency_ms) / self.stats["misses"]
            )
        else:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        hit_rate = 0.0
        if total > 0:
            hits = self.stats["l1_hits"] + self.stats["l2_hits"] + self.stats["negative_hits"]
            hit_rate = hits / total
