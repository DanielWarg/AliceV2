...
class ShadowResponse(BaseModel):
    ...
    canary_eligible: bool = Field(
        default=False, description="Whether response is eligible for canary"
    )
    canary_routed: bool = Field(
        default=False, description="Whether response was actually routed via canary"
    )
...
class CanaryConfig(BaseModel):
    ...
    min_schema_ok: float = Field(
        default=0.95, description="Minimum schema_ok rate for canary"
    )
    min_intent_match: float = Field(
        default=0.90, description="Minimum intent match rate for canary"
    )
    evaluation_window_hours: int = Field(
        default=24, description="Evaluation window in hours"
    )
    allowed_levels: list[str] = Field(
        default=["easy", "medium"], description="Levels eligible for canary"
    )
    max_latency_delta_ms: float = Field(
        default=150.0, description="Maximum latency increase allowed"
    )
...
class PlannerComparison(BaseModel):
    ...
    canary_routed: bool = Field(
        description="Whether request was actually routed via canary"
    )
    rollback_reason: Optional[str] = Field(
        default=None, description="Reason for rollback if any"
    )
