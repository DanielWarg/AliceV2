...
from .models import (
    ShadowRequest, ShadowResponse, CanaryConfig, 
    PlannerComparison, ShadowEvent
)
...
class ShadowEvaluator:
    ...
    def compare_responses(self, primary_result: Dict[str, Any], shadow_result: Dict[str, Any]) -> PlannerComparison:
        ...
        level = primary_result.get("level", complexity.lower())  # Map HARD -> hard, MEDIUM -> medium, etc.
        ...
        primary_args = self._canonical_args(primary_result.get("args", {}), primary_tool)
    ...
    def log_event(self, event: ShadowEvent) -> None:
        ...
        try:
            # Write to JSONL file
            event_file = self.eval_dir / f"shadow_events_{datetime.utcnow().strftime('%Y-%m-%d')}.jsonl"
...
class CanaryRouter:
    ...
    def should_route_canary(self, session_id: str, comparison: PlannerComparison) -> bool:
    ...
    async def evaluate_shadow(
        ...
        canary_routed = canary_eligible and comparison.schema_ok_both and comparison.intent_match
