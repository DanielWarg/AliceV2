...
class ChatRequest(BaseRequest):
    ...
    model: Optional[ModelType] = Field(
        default=ModelType.AUTO, description="Preferred model"
    )
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional context"
    )
    force_route: Optional[str] = Field(
        default=None, description="Force specific route (micro, planner, deep)"
    )
...
class IngestRequest(BaseRequest):
    ...
    intent: Optional[str] = Field(
        default=None, description="Detected intent (if known)"
    )
    context: Optional[Dict[str, Any]] = Field(
        default=None, description="Request context"
    )
