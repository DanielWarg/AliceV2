...
class NLUResult:
    ...
    def __init__(self, intent: str, confidence: float, slots: Dict[str, Any] = None, 
                 route_hint: str = "planner", source: str = "nlu", validated: bool = False):
        self.intent = intent
...
class NLUClient:
    ...
    async def parse(self, text: str, lang: str = "sv") -> NLUResult:
        ...
        try:
            ...
            logger.warn("NLU service call failed - using fallback", 
                       error=str(e), text=text[:50])
            return self._fallback_parse(text)
    ...
    def _fallback_parse(self, text: str) -> NLUResult:
        ...
        for keywords, (intent, route_hint, confidence) in self.fallback_patterns.items():
            if any(keyword in text_lower for keyword in keywords):
                logger.info("Fallback NLU match", 
                          intent=intent, 
