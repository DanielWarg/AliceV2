...
class MemoryClient:
    ...
    async def query_memory(
        self, user_id: str, query: str, top_k: int = 3, min_score: float = 0.6
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to query memory", user_id=user_id, query=query, error=str(e)
            )
            raise
    ...
    async def forget_memory(
        ...
        try:
            ...
            response = await self.client.post(f"{self.base_url}/forget", json=payload)
            response.raise_for_status()
