...
def classify_tool_error_reason(status: int|None, exc: Exception|None) -> str:
    if exc and "timeout" in str(exc).lower(): return "timeout"
    if status == 429: return "429"
    if status and 500 <= status <= 599: return "5xx"
    if exc and "schema" in str(exc).lower(): return "schema"
    return "other"
...
def gate_tool_call(tool_name: str, args: dict, mode: str, policy: dict, model: type[BaseModel]|None=None):
    if model:
