...
TOOL_MAP = {
    "time.now": [
        "time", "klockan", "vad är klockan", "hur mycket är klockan",
        "tid", "time", "vilken tid", "nuvarande tid", "current time",
        "klockan?", "tid nu", "nuvarande tid", "vad är tiden?"
    ],
    "weather.lookup": [
        "weather", "väder", "temperaturen", "vädret", "weather lookup",
        "vad är vädret", "hur blir vädret", "vädret i", "weather in",
        "kolla vädret", "check weather", "temperatur", "regnar det"
    ],
    "memory.query": [
        "memory", "minns", "kom ihåg", "spara", "memory query",
        "kommer du ihåg", "vad sa vi", "minne", "remember",
        "vad pratade vi", "vad diskuterade vi", "spara detta"
    ],
    "greeting.hello": [
        "greeting", "hej", "hello", "hi", "tjena", "hallå",
        "god morgon", "god afton", "god kväll", "trevligt att träffas",
        "good morning", "good evening", "hej!", "hallå"
    ],
    "calendar.create_draft": [
        "calendar", "boka möte", "skapa möte", "boka tid", "calendar create draft",
        "schedule meeting", "book meeting", "boka konferensrum", "planera möte",
        "skapa kalender", "boka rum", "schedule appointment", "boka"
    ],
    "email.create_draft": [
        "email", "skicka email", "skicka mail", "maila till", "email create draft",
        "send email", "skriv ett mail", "skicka rapport", "email till",
        "skicka meddelande", "skriv email", "send mail", "maila"
    ]
}
...
def test_tool_selector():
    ...
    for text, expected in test_cases:
        ...
        print(f"{status} '{text}' → {result} (expected: {expected}, confidence: {confidence:.1%})")
