...
class GracefulShutdown:
    ...
    def register_request(self, request_id: str):
        ...
        if not self.shutdown_requested:
            ...
            logger.debug("Request registered", request_id=request_id, active_count=len(self.active_requests))
    
    def unregister_request(self, request_id: str):
        """Unregister a completed request"""
        if request_id in self.active_requests:
            self.active_requests.remove(request_id)
            logger.debug("Request completed", request_id=request_id, active_count=len(self.active_requests))
    ...
    def start_shutdown(self):
        ...
        if not self.shutdown_started:
            ...
            logger.warning("Shutdown started - draining requests", active_count=len(self.active_requests))
    ...
    async def wait_for_drain(self) -> bool:
        ...
        if self.active_requests:
            logger.error("Shutdown timeout - forcing shutdown", 
                        active_count=len(self.active_requests))
            return False
        else:
