...
from ..planner.schema_v4 import PlanOut
...
Analysera noggrant och välj det mest lämpliga intent och tool. Svara ENDAST med JSON."""
    ...
    def generate(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if not classification.use_llm:
                ...
                v4_result = {
                    "intent": (
                        classification.tool.split(".")[0]
                        if "." in classification.tool
                        else "none"
                    ),
                    "tool": classification.tool,
                ...
                return {
                    "text": json.dumps(
                        v4_result, separators=(",", ":"), ensure_ascii=False
                    ),
                    "model": "classifier_v2",
                    "tokens_used": 0,
                    "temperature": 0.0,
            ...
            logger.info(
                "Generating planner v2 response",
                model=self.model,
                ...
                try:
                    ...
                    if attempt == 0 and not auto_repaired:
                        ...
                        if last_brace > 0:
                            ...
                            auto_repaired = True
                            logger.warning(
                                "planner_v2.auto_repair_applied",
                                pos=e.pos,
                                msg=str(e),
                    ...
                    logger.warning(
                        "planner_v2.json_decode_error_final",
                        pos=e.pos,
                        msg=str(e),
                        sample=(processed_text or "")[:200],
                    )
            ...
            try:
                ...
                if "tool" in parsed_json and "args" in parsed_json:
                    parsed_json["args"] = PlanOut.canonicalize_args(
                        parsed_json["tool"], parsed_json["args"]
                    )
                ...
                logger.warning(
                    "planner_v2.schema_validation_failed", err=str(e), obj=parsed_json
                )
                ...
                if "tool" in repaired_json and "args" in repaired_json:
                    repaired_json["args"] = PlanOut.canonicalize_args(
                        repaired_json["tool"], repaired_json["args"]
                    )
                ...
                try:
                    ...
                    logger.info(
                        "planner_v2.auto_repair_successful", repaired_json=repaired_json
                    )
                except ValidationError as repair_error:
                    logger.warning(
                        "planner_v2.auto_repair_failed",
                        err=str(repair_error),
                        obj=repaired_json,
                    )
            ...
            logger.info(
                "Planner v2 response generated successfully",
                model=self.model,
    ...
    def _repair_enum_values(self, parsed_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(render_instruction, str):
            if (
                "<" in render_instruction
                or ">" in render_instruction
                or render_instruction == "<enum>"
            ):
    ...
    def _fallback_response(self, reason: str) -> Dict[str, Any]:
        ...
        return {
            "text": json.dumps(
                fallback_json, separators=(",", ":"), ensure_ascii=False
            ),
            "model": "fallback_v2",
            "tokens_used": 0,
            "temperature": 0.0,
