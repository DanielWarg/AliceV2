...
Ge detaljerade, välstrukturerade svar för komplexa frågor."""
        ...
        if not self._initialized:
            ...
            try:
                ...
                if self.model not in models:
                    logger.warning("Deep model not available, will use fallback", model=self.model)
                    return False
                ...
                logger.error("Failed to initialize deep model", model=self.model, error=str(e))
                return False
    ...
    def _check_guardian_allowance(self) -> bool:
        ...
        try:
            ...
            if health.get("state") != "NORMAL":
                logger.warning("Deep model blocked by Guardian", 
                             guardian_state=health.get("state"))
                return False
            
            return True
            
        except Exception as e:
            logger.warning("Could not check Guardian state, allowing deep model", error=str(e))
            return True
    ...
    def generate(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("Generating deep response", model=self.model, prompt_length=len(prompt))
            ...
            logger.info("Deep response generated", 
                       model=self.model, 
