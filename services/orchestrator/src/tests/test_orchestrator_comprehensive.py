...
class TestOrchestratorCore:
    ...
    def test_orchestrator_health_states(self, mock_get_health, client, test_metrics):
        ...
        for i, state_config in enumerate(guardian_states):
            ...
            test_metrics(
                f"guardian_state_{state_config['state']}_status", response.status_code
            )
        ...
        ), f"Guardian response too slow: {avg_response_time}ms > 150ms"
        assert (
            compliance_rate == 1.0
        ), f"Guardian state compliance failed: {compliance_rate}"

    @patch("src.services.guardian_client.GuardianClient.check_admission")
    @patch("src.services.guardian_client.GuardianClient.get_health")
    def test_api_versioning_compliance(
        self, mock_get_health, mock_check_admission, client, test_metrics
    ):
        ...
        for i, scenario in enumerate(version_scenarios):
            ...
            test_metrics(
                f"version_{scenario.get('v', 'missing')}_response_ms", response_time
            )
            test_metrics(
                f"version_{scenario.get('v', 'missing')}_status", response.status_code
            )
        ...
        assert (
            compliance_rate == 1.0
        ), f"API version compliance failed: {compliance_rate}"
        assert (
            avg_response_time < 100
        ), f"API version response too slow: {avg_response_time}ms"
...
class TestOrchestratorPerformance:
    ...
    def test_chat_api_slo_compliance(
        self,
        mock_get_model,
        mock_get_health,
        ...
        # Warm up request
        client.post(
            "/api/chat", json={"v": "1", "session_id": "warmup", "message": "warmup"}
        )
    ...
    def test_concurrent_request_handling(
        ...
        for result in request_results:
            test_metrics(
                f"concurrent_req_{result['request_id']}_ms", result["response_time"]
            )
        ...
        ), f"Concurrent avg response too slow: {avg_response_time}ms"
...
class TestOrchestratorResilience:
    ...
    def test_malformed_request_handling(self, client, test_metrics):
        ...
        malformed_scenarios = [
            ...
            {
                "json": {"v": "1", "session_id": "test", "message": "x" * 10000},
                "expected_status": 422,
        ...
        for i, scenario in enumerate(malformed_scenarios):
            ...
            test_metrics(
                f"error_handling_{scenario['error_type']}_status", response.status_code
            )
        ...
        assert (
            resilience_rate == 1.0
        ), f"Error handling resilience failed: {resilience_rate}"
        assert (
            avg_error_response < 100
        ), f"Error response too slow: {avg_error_response}ms"
    ...
    def test_guardian_failure_scenarios(self, mock_get_health, client, test_metrics):
        ...
        guardian_failure_scenarios = [
            {
                "side_effect": ConnectionError("Guardian unreachable"),
                "expected_status": 503,
            },
            {"side_effect": TimeoutError("Guardian timeout"), "expected_status": 503},
            {
                "return_value": {"state": "UNKNOWN", "available": False},
                "expected_status": 503,
            },
        ]
        ...
        avg_failure_response = total_failure_response_time / len(
            guardian_failure_scenarios
        )
        ...
        assert (
            failure_handling_rate == 1.0
        ), f"Guardian failure handling failed: {failure_handling_rate}"
        assert (
            avg_failure_response < 500
        ), f"Guardian failure response too slow: {avg_failure_response}ms"
    ...
    def test_structured_logging_compliance(self, client, test_metrics):
        ...
        test_requests = [
            ...
            {"endpoint": "/health", "method": "GET"},
            {
                "endpoint": "/api/chat",
                "method": "POST",
                "json": {"v": "1", "session_id": "log_test", "message": "test"},
            },
        ]
        ...
        for request in test_requests:
            ...
            test_metrics(
                f"logging_{request['endpoint'].replace('/', '_')}_response_ms",
                response_time,
            )
            test_metrics(
                f"logging_{request['endpoint'].replace('/', '_')}_has_trace_id",
                int(has_trace_id),
            )

        # Verify trace ID propagation
        chat_response = client.post(
            "/api/chat", json={"v": "1", "session_id": "trace_test", "message": "trace"}
        )
