...
class TestMetricsCollector:
    ...
    def __init__(self):
        ...
        base_dir = (
            Path(__file__).parent.parent.parent.parent.parent / "test-results"
        )  # Absolute path to repo root/test-results
    ...
    def pytest_runtest_logreport(self, report):
        ...
        if report.when == "call":  # Only log the actual test call, not setup/teardown
            ...
            if report.failed and report.longrepr:
                test_data["failure_reason"] = (
                    str(report.longrepr).split("\n")[-2]
                    if "\n" in str(report.longrepr)
                    else str(report.longrepr)
                )
                test_data["failure_type"] = (
    ...
    def pytest_sessionfinish(self, session, exitstatus):
        ...
        session_summary = {
            ...
            "test_efficiency_score": passed_tests
            / (total_duration + 1),  # Tests per second
        }
