...
class RealIntegrationTestSuite:
    ...
    def setup_class(cls):
        ...
        cls.orchestrator_client = httpx.Client(base_url=ORCHESTRATOR_URL, timeout=TEST_TIMEOUT)
        ...
        try:
            ...
            assert response.status_code == 200, f"Orchestrator not running: {response.status_code}"
...
class TestRealisticOrchestratorAPI(RealIntegrationTestSuite):
    ...
    def test_orchestrator_basic_health_check(self):
        ...
        # Log real test data
        log_test_event("orchestrator_health_check", 
    ...
    def test_chat_api_realistic_flow(self):
        ...
        for i, case in enumerate(test_cases):
            ...
            log_test_event(f"chat_realistic_{i}",
        ...
        # Performance should be reasonable
        assert avg_response_time < 2000, f"Average response too slow: {avg_response_time}ms"
    ...
    def test_api_version_handling_realistic(self):
        ...
        for scenario in test_scenarios:
            ...
            log_test_event("api_version_handling",
                          version=scenario.get("v", "missing"),
                          status_code=response.status_code,
                          expected_success=scenario["should_work"])
        
        # At least valid version should work
        valid_version_worked = any(r["status_code"] == 200 for r in results if r["scenario"] == "Valid version")
    ...
    def test_concurrent_load_realistic(self):
        ...
        log_test_event("concurrent_load_realistic",
        ...
        if response_times:
            assert avg_response_time < 5000, f"Response time under load too slow: {avg_response_time}ms"

class TestGuardianIntegration(RealIntegrationTestSuite):
    """Test real Guardian service integration"""
    
    @pytest.mark.skipif(not hasattr(RealIntegrationTestSuite, 'guardian_available') or 
                       not RealIntegrationTestSuite.guardian_available,
    ...
    def test_guardian_health_states_realistic(self):
        ...
        if guardian_response.status_code == 200:
            ...
            log_test_event("guardian_health_realistic",
    ...
    def test_guardian_resource_awareness(self):
        ...
        if guardian_response.status_code == 200:
            ...
            chat_response = self.orchestrator_client.post("/api/chat", json={
                "v": "1",
                "session_id": "resource_test",
                "message": "Test under current resource conditions"
            })
            
            log_test_event("guardian_resource_awareness",
...
class TestErrorHandlingRealistic(RealIntegrationTestSuite):
    ...
    def test_malformed_requests_graceful_handling(self):
        ...
        test_scenarios = [
            ...
            {"json": {"v": "1", "session_id": ""}, "severity": "MEDIUM", "should_reject": True},
            {"json": {"v": "1", "session_id": "test", "message": ""}, "severity": "MEDIUM", "should_reject": True},
            {"json": {"v": "1", "session_id": "test", "message": "x" * 5000}, "severity": "LOW", "should_reject": False},
            {"json": {"v": "1", "session_id": "test", "message": "Normal message"}, "severity": "NONE", "should_reject": False}
        ]
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            properly_handled = (
                (scenario["should_reject"] and response.status_code >= 400) or
                (not scenario["should_reject"] and response.status_code == 200)
            )
            
            if is_critical and properly_handled:
                critical_handled += 1
            
            log_test_event(f"malformed_request_{i}",
        ...
        if total_critical > 0:
            ...
            assert critical_success_rate >= 0.9, f"Critical error handling too low: {critical_success_rate}"
    ...
    def test_network_resilience(self):
        ...
        for timeout in timeouts:
            ...
            try:
                ...
                log_test_event("network_resilience",
                              timeout_s=timeout,
                              status_code=response.status_code,
                              response_time_ms=response_time,
                              success=True)
                ...
                if timeout >= 1.0:
                    pytest.fail(f"Request failed with reasonable timeout {timeout}s: {e}")
...
class TestEndToEndUserJourneys(RealIntegrationTestSuite):
    ...
    def test_swedish_conversation_flow(self):
        ...
        for i, turn in enumerate(conversation):
            ...
            log_test_event(f"swedish_conversation_turn_{i}",
        ...
        assert conversation_success_rate >= 0.75, f"Conversation success rate too low: {conversation_success_rate}"
    ...
    def test_error_recovery_flow(self):
        ...
        bad_response = self.orchestrator_client.post("/api/chat", json={
            "v": "1", 
            "session_id": session_id,
        ...
        good_response = self.orchestrator_client.post("/api/chat", json={
            "v": "1",
            "session_id": session_id, 
            "message": "Hej, fungerar systemet nu?"
        })
        
        log_test_event("error_recovery_flow",
        ...
        assert good_response.status_code == 200, "System should recover after handling bad request"