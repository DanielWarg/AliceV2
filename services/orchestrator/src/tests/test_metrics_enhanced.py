...
class TestOrchestrator_Performance:
    ...
    def test_chat_performance_under_load(
        ...
        # Warm up
        client.post(
            "/api/chat", json={"v": "1", "session_id": "warmup", "message": "test"}
        )
        ...
        ), f"P95 response time {p95_response_time}ms too high"
    ...
    def test_orchestrator_routing_accuracy(self, client, test_metrics):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            if response.status_code == 200:
                ...
                test_metrics(
                    f"routing_{i}_estimated_latency", data.get("estimated_latency_ms")
                )
        ...
        ), f"Routing accuracy {routing_accuracy_rate} too low"
    ...
    def test_error_handling_resilience(self, client, test_metrics):
        ...
        ), f"Error handling accuracy {error_handling_rate} too low"
        assert (
            avg_error_response_time < 100
        ), f"Error response time {avg_error_response_time}ms too high"

    @patch("src.services.guardian_client.GuardianClient.get_health")
    def test_guardian_integration_reliability(
        self, mock_get_health, client, test_metrics
    ):
        ...
        # Assertions
        assert (
            guardian_integration_rate >= 0.9
        ), f"Guardian integration reliability {guardian_integration_rate} too low"
        assert (
            avg_guardian_time < 150
        ), f"Guardian integration time {avg_guardian_time}ms too high"
