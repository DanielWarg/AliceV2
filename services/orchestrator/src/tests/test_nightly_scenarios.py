...
class NightlyTestSuite:
    ...
    def setup_class(cls):
        ...
        cls.config = cls.test_data["alice_v2_production_test_scenarios"][
            "test_execution_config"
        ]
        cls.thresholds = cls.test_data["alice_v2_production_test_scenarios"][
            "success_thresholds"
        ]
    ...
    def _verify_system_health(cls):
        ...
        try:
            ...
            ), f"Orchestrator unhealthy: {response.status_code}"
    ...
    def _generate_nightly_summary(cls):
        ...
        successful_scenarios = sum(
            1 for r in cls.test_results if r.get("overall_success", False)
        )
        ...
        response_times = [
            r.get("total_response_time_ms", 0)
            ...
            "nightly_test_summary": {
                ...
                "slo_compliance": {
                    "overall_success_rate": {
                        ...
                        "compliant": (
                            (successful_scenarios / total_scenarios)
                            >= cls.thresholds["overall_success_rate"]
                            if total_scenarios > 0
                            else False
                        ),
                    },
                    "response_time_p95": {
                        "target_ms": cls.test_data[
                            "alice_v2_production_test_scenarios"
                        ]["metadata"]["slo_targets"]["response_time_p95_ms"],
                        "actual_ms": p95,
                        "compliant": p95
                        <= cls.test_data["alice_v2_production_test_scenarios"][
                            "metadata"
                        ]["slo_targets"]["response_time_p95_ms"],
                    },
                },
        ...
        print(
            f"   Success Rate: {successful_scenarios}/{total_scenarios} ({successful_scenarios/total_scenarios:.1%})"
        )
        print(f"   P95 Response Time: {p95:.0f}ms")
        print(
            f"   Test Duration: {(time.time() - cls.session_start_time)/60:.1f} minutes"
        )
    ...
    def _execute_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with logging_context(f"nightly_{scenario_id}", "system_test"):
            ...
            try:
                ...
                result["overall_success"] = self._evaluate_success_criteria(
                    scenario, result
                )
        ...
        for turn_idx, turn in enumerate(scenario["conversation"]):
            ...
            try:
                ...
                turn_result = {
                    ...
                    "within_time_limit": response_time_ms
                    <= turn.get("max_response_time_ms", 5000),
                }
    ...
    def _execute_concurrent_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def run_concurrent_user(user_id):
            session_id = (
                f"nightly_concurrent_{scenario['id']}_{user_id}_{int(time.time())}"
            )

            # Modify user message to include session identifier
            user_message = scenario["conversation"][0]["user"].format(
                session_id=user_id
            )
            ...
            try:
                response = self.orchestrator_client.post(
                    "/api/chat",
                    json={"v": "1", "session_id": session_id, "message": user_message},
                )
        ...
        with ThreadPoolExecutor(max_workers=concurrent_sessions) as executor:
            futures = [
                executor.submit(run_concurrent_user, i)
        ...
        if successful_sessions:
            avg_response_time = sum(
                r["response_time_ms"] for r in successful_sessions
    ...
    def _evaluate_success_criteria(
        ...
        if "conversation_completion" in criteria:
            ...
            if turns:
                ...
                met_criteria["conversation_completion"] = (
        ...
        if "intent_confidence" in criteria:
            ...
            met_criteria["intent_confidence"] = (
                result.get("success_rate", 0) > 0
                or len([t for t in result.get("turns", []) if t.get("success")]) > 0
            )
...
class TestProductionScenarios(NightlyTestSuite):
    ...
    def test_production_scenario(self, scenario):
        ...
        if scenario["priority"] == "CRITICAL":
            assert result[
                "overall_success"
            ], f"Critical scenario {scenario['id']} failed: {result}"
        elif scenario["priority"] == "HIGH":
            # Allow some flexibility for high priority
            if not result["overall_success"]:
                print(
                    f"⚠️  High priority scenario {scenario['id']} failed - investigating"
                )

        # Log success for monitoring
        assert (
            "overall_success" in result
        ), f"Scenario {scenario['id']} didn't complete properly"
...
class TestNightlyAggregation(NightlyTestSuite):
    ...
    def test_overall_slo_compliance(self):
        ...
        successful_scenarios = sum(
            1 for r in self.test_results if r.get("overall_success", False)
        )
        overall_success_rate = successful_scenarios / total_scenarios

        # Check critical scenarios specifically
        critical_results = [
            r for r in self.test_results if r.get("priority") == "CRITICAL"
        ]
        if critical_results:
            critical_success_rate = sum(
                1 for r in critical_results if r.get("overall_success", False)
            ) / len(critical_results)
            assert (
                critical_success_rate
                >= self.thresholds["critical_scenarios_success_rate"]
            ), f"Critical scenarios success rate too low: {critical_success_rate:.2%}"

        # Overall success rate
        assert (
            overall_success_rate >= self.thresholds["overall_success_rate"]
    ...
    def test_performance_regression_detection(self):
        ...
        target_p95 = self.test_data["alice_v2_production_test_scenarios"]["metadata"][
            "slo_targets"
        ...
        if not performance_acceptable:
            print(
                f"⚠️  Performance regression detected: P95 {current_p95:.0f}ms vs target {target_p95}ms"
            )
        ...
        assert (
            current_p95 <= target_p95 * 2.0
        ), f"Severe performance regression: P95 {current_p95:.0f}ms >> target {target_p95}ms"
