...
from src.utils.data_collection import log_test_event, log_test_failure
...
class ChaosTestSuite:
    ...
    def _measure_baseline_performance(cls) -> float:
        ...
        for _ in range(5):
            ...
            response = cls.orchestrator_client.post("/api/chat", json={
                "v": "1",
                "session_id": "baseline",
...
class TestNetworkChaos(ChaosTestSuite):
    ...
    def test_connection_timeout_resilience(self):
        ...
        for scenario in timeout_scenarios:
            ...
            log_test_event(f"connection_timeout_{timeout}",
                          timeout_s=timeout,
                          success_rate=success_rate,
        ...
        for result in reasonable_timeouts:
            assert result["success_rate"] >= 0.8, f"Success rate too low for {result['timeout']}s timeout: {result['success_rate']}"
    ...
    def test_connection_pool_exhaustion(self):
        ...
        with ThreadPoolExecutor(max_workers=concurrent_requests) as executor:
            futures = [executor.submit(make_long_request, i) for i in range(concurrent_requests)]
        ...
        log_test_event("connection_pool_exhaustion",
                      concurrent_requests=concurrent_requests,
                      success_rate=success_rate,
                      avg_response_time_ms=avg_response_time)
        
        # Under extreme load, some failures are acceptable but system should not completely fail
        assert success_rate >= 0.3, f"Success rate under connection pressure too low: {success_rate}"
...
class TestResourceChaos(ChaosTestSuite):
    ...
    def test_memory_pressure_behavior(self):
        ...
        def memory_pressure_request(request_id):
            try:
                response = self.orchestrator_client.post("/api/chat", json={
                    ...
                    "message": f"Memory pressure test {request_id} - " + "X" * 1000  # Larger payload
                })
        ...
        with ThreadPoolExecutor(max_workers=concurrent_requests) as executor:
            futures = [executor.submit(memory_pressure_request, i) for i in range(concurrent_requests)]
        ...
        log_test_event("memory_pressure_behavior",
        ...
        assert success_rate >= 0.7, f"Success rate under memory pressure too low: {success_rate}"
    ...
    def test_cpu_intensive_requests(self):
        ...
        with ThreadPoolExecutor(max_workers=concurrent_requests) as executor:
            futures = [executor.submit(cpu_intensive_request, i) for i in range(concurrent_requests)]
            results = [future.result() for future in futures]
        ...
        log_test_event("cpu_intensive_requests",
                      concurrent_requests=concurrent_requests,
                      success_rate=success_rate,
                      avg_response_time_ms=avg_response_time,
        ...
        assert success_rate >= 0.75, f"Success rate for CPU-intensive requests too low: {success_rate}"
        ...
        if avg_response_time > 0:
            ...
            assert avg_response_time < 10000, f"Average response time under CPU load too high: {avg_response_time}ms"
    ...
    def test_increasing_load_degradation(self):
        ...
        for load_level in load_levels:
            ...
            with ThreadPoolExecutor(max_workers=load_level) as executor:
                futures = [executor.submit(load_test_request, i) for i in range(load_level)]
            ...
            log_test_event(f"load_degradation_{load_level}",
                          load_level=load_level,
                          success_rate=success_rate,
                          avg_response_time_ms=avg_response_time,
        ...
        for result in results:
            print(f"Load {result['load_level']:2d}: {result['success_rate']:.1%} success, {result['avg_response_time']:.0f}ms avg")
        
        # Light loads should work well
        light_loads = [r for r in results if r["load_level"] <= 5]
        for result in light_loads:
            assert result["success_rate"] >= 0.9, f"Light load performance poor at level {result['load_level']}"
        ...
        if heavy_loads:
            ...
            assert worst_heavy_load["success_rate"] >= 0.3, "System should not fail completely under heavy load"
...
class TestRecoveryScenarios(ChaosTestSuite):
    ...
    def test_error_spike_recovery(self):
        ...
        for i in range(10):
            try:
                response = self.orchestrator_client.post("/api/chat", json={
                    "invalid": "request_structure"  # Intentionally malformed
                })
        ...
        if successful_recovery:
            avg_recovery_time = sum(r["response_time"] for r in successful_recovery) / len(successful_recovery)
        else:
            avg_recovery_time = 0
        
        log_test_event("error_spike_recovery",
        ...
        assert recovery_rate >= 0.8, f"Recovery rate after error spike too low: {recovery_rate}"
        ...
        if avg_recovery_time > 0:
            ...
            assert slowdown_factor < 3, f"Recovery too slow compared to baseline: {slowdown_factor:.1f}x"
    ...
    def test_sustained_load_recovery(self):
        ...
        log_test_event("sustained_load_recovery",
        ...
        print(f"📊 Recovery: {recovery_rate:.1%} success rate, {avg_recovery_time:.0f}ms avg time")
        
        # System should recover after sustained load
        assert recovery_rate >= 0.8, f"Recovery rate after sustained load too low: {recovery_rate}"