...
from ..cache.smart_cache import get_smart_cache, CacheResult
from ..clients.nlu_client import get_nlu_client, NLUResult
from ..router.policy import get_router_policy
from ..utils.circuit_breaker import get_circuit_breaker, CircuitBreakerConfig, CircuitOpenError
from ..llm.micro_client import RealMicroClient, MockMicroClient
...
class OptimizedOrchestrator:
    ...
    def __init__(self):
        ...
        self.planner_circuit_breaker = get_circuit_breaker("planner_service", planner_cb_config)
    ...
    async def process_request(self, request: ChatRequest) -> ChatResponse:
        ...
        logger.info("Processing optimized request", 
                   trace_id=trace_id,
        ...
        try:
            ...
            cache_result = await self.smart_cache.get("unknown", request.text)
            ...
            if cache_result.hit:
                ...
                return ChatResponse(
                    response=cache_result.data.get("render_instruction", {}).get("content", "Cached response"),
            ...
            nlu_task = asyncio.create_task(self._process_nlu(request.text))
            
            # STEP 3: Route decision based on NLU and text analysis
            nlu_result = await nlu_task
            route_decision = self.router_policy.decide_route(request.text)
            ...
            if route_decision.route == "micro":
                ...
                response_data = await self._execute_planner(request, nlu_result, trace_id)
            else:  # deep route
            ...
            if response_data and "error" not in response_data:
                try:
                    self.smart_cache.set(
                        ...
                        request.text, 
                        response_data,
                        ttl=300 if nlu_result.intent in ["time.now", "weather.lookup"] else 600
                    )
            ...
            final_response = ChatResponse(
                response=response_data.get("render_instruction", {}).get("content", "Response generated"),
                model=response_data.get("meta", {}).get("model_id", route_decision.route),
                meta={
                    "trace_id": trace_id,
                    "route": route_decision.route,
                    ...
                    "tool_precision": response_data.get("meta", {}).get("tool_precision", 1.0),
            ...
            self.smart_cache.set_negative(request.text, "error", ttl=60)
    ...
    async def _execute_micro(self, request: ChatRequest, nlu_result: NLUResult, trace_id: str) -> Dict[str, Any]:
        """Execute with micro model"""
        
        micro_start = time.perf_counter()
        
        try:
            response = self.micro_client.generate(request.text)
            micro_ms = (time.perf_counter() - micro_start) * 1000
            
            logger.info("Micro execution completed", 
                       trace_id=trace_id,
    ...
    async def _execute_planner(self, request: ChatRequest, nlu_result: NLUResult, trace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warn("Planner circuit breaker open - using fallback", trace_id=trace_id)
    ...
    def _call_planner_service(self, request: ChatRequest, nlu_result: NLUResult, trace_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "render_instruction": {
                "type": "text",
                "content": f"Planner hanterar: {request.text}"
            },
            "meta": {
                "route": "planner",
    ...
    async def _execute_deep(self, request: ChatRequest, nlu_result: NLUResult, trace_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "tool": "deep_analysis",
            "args": {"complex_query": request.text},
            "render_instruction": {
                "type": "text",
                "content": f"Deep analysis av: {request.text}"
            },
            "meta": {
                "route": "deep",
                "latency_ms": deep_ms,
