...
async def system_health():
    ...
    try:
        ...
        with httpx.Client(timeout=2.0) as client:
            ...
            health_status["ollama_models"] = [model["name"] for model in response.json().get("models", [])]
    except Exception as e:
...
async def performance_metrics():
    ...
    return {
        ...
        "current_metrics": {
            ...
            "micro_routing_share": quota_stats.get("micro_routing", {}).get("micro_share", 0.0)
        },
        "recommendations": _get_performance_recommendations(performance_score, cache_stats, quota_stats)
    }
...
def _get_routing_recommendations(quota_stats: Dict[str, Dict]) -> list:
    ...
    if micro_share > target_share + 0.1:
        ...
        recommendations.append({
            "level": "info", 
            "message": f"Micro routing below target - could increase for better latency"
        })
    else:
        recommendations.append({
            "level": "info",
            "message": "Routing distribution is optimal"
        })
...
def _calculate_performance_score(cache_stats: Dict, quota_stats: Dict) -> float:
    ...
    routing_efficiency = max(0, 1.0 - abs(actual_share - target_share) * 2) * 20  # Max 20 points
...
def _get_performance_recommendations(score: float, cache_stats: Dict, quota_stats: Dict) -> list:
    ...
    if score < 60:
        ...
        recommendations.append({
            "level": "warning",
            "message": "System performance needs optimization"  
        })
    else:
        recommendations.append({
            "level": "info",
