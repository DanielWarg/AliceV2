...
def get_api_key(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> str:
    ...
    if api_key not in VALID_API_KEYS:
        logger.warning("Invalid API key attempted", api_key_hash=hashlib.sha256(api_key.encode()).hexdigest()[:8])
        raise HTTPException(
            status_code=401,
...
def check_rate_limit(request: Request, api_key: str) -> None:
    ...
    if rate_key in rate_limit_store:
        while rate_limit_store[rate_key] and rate_limit_store[rate_key][0] < window_start:
