...
def analyze_training_data(examples: list) -> dict:
    ...
    for ex in examples:
        intent = (
            ex["input"].split("Intent: ")[1].split("\n")[0]
            if "Intent: " in ex["input"]
            else "unknown"
        )
    ...
    analysis = {
        ...
        "quality": (
            "high"
            if success_rate > 0.7 and avg_reward > 3.0
            else "medium" if success_rate > 0.4 else "poor"
        ),
...
def simulate_training_process(examples: list, output_dir: Path) -> dict:
    ...
    logger.info("✅ ToolSelector training completed successfully!")
    logger.info(
        f"📊 Final accuracy: {training_metadata['final_metrics']['train_accuracy']:.2%}"
    )
...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default=str(DATA_DIR / "rl" / "v1"),
        help="Training data directory",
    )
    parser.add_argument(
        "--output-dir",
        default=str(WEIGHTS_DIR / "toolselector" / "v1"),
        help="Output directory",
    )
    parser.add_argument("--component", default="toolselector", help="Component name")
    parser.add_argument(
        "--max-time-hours", type=float, default=0.5, help="Max training time"
    )
    ...
    if result["success"]:
        ...
        print(
            f"Final accuracy: {result['metadata']['final_metrics']['train_accuracy']:.2%}"
        )
