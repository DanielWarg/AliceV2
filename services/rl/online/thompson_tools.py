...
class ThompsonTools:
    ...
    def __init__(self):
        ...
        if self.state is None:
            ...
            print(
                f"Loaded Thompson tool selector with {len(self.state.policies)} intents, {total_policies} total policies"
            )
    ...
    def select(
        ...
        if available_tools is not None:
            tools_dict = {
                t: arm for t, arm in tools_dict.items() if t in available_tools
            }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {"total_updates": self.update_count, "intents": {}}

        for intent, tools in self.state.policies.items():
            intent_stats = {"num_tools": len(tools), "tools": {}}
    ...
    def get_tool_probabilities(
        self, intent: str, num_samples: int = 1000
    ) -> Dict[str, float]:
