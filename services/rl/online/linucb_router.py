...
from services.rl.persistence.bandit_store import (
    LinUCBState, LinUCBArm, load_linucb, save_linucb, init_empty_linucb
)
...
class LinUCBRouter:
    ...
    def __init__(self, actions: Optional[List[str]] = None, alpha: Optional[float] = None, dim: int = FEATURE_DIM):
        ...
        if self.state is None:
            ...
            print(f"Loaded LinUCB router with {len(self.state.arms)} arms, {self._total_pulls()} total pulls")
    ...
    def _extract_features(self, context: Dict[str, Any]) -> np.ndarray:
        ...
        len_chars = min(float(context.get("len_chars", 0)) / 500.0, 1.0)  # normalize to 0..1
        ...
        ][:self.dim])  # Clip to actual dimension
    ...
    def update(self, context: Dict[str, Any], action: str, reward: float) -> None:
        ...
        if self.update_count > 100:
            self.state.alpha = max(0.1, DEFAULT_ALPHA * np.exp(-self.update_count / 1000))
    ...
    def get_action_probabilities(self, context: Dict[str, Any], temperature: float = 1.0) -> Dict[str, float]:
