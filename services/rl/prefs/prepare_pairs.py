#!/usr/bin/env python3
import json, argparse, re, os, random
from pathlib import Path

def score_phi(meta, cfg):
    phi = cfg.get("phi_weights", {})
    correctness = 1.0 if not meta.get("error") else 0.0
    brevity = 1.0 if meta.get("len", 0) <= cfg.get("labeling",{}).get("brevity_target_chars",450) else 0.5
    ...
    return (phi.get("correctness",0)*correctness +
...
def main():
    ...
    patterns_banned = [re.compile(pat, re.I) for pat in cfg.get("labeling",{}).get("banned_patterns",[])]
    ...
    with open(args.inp, "r", encoding="utf-8") as fin, open(args.out, "w", encoding="utf-8") as fout:
        for line in fin:
            ...
            a = ex.get("resp_a") or (ex.get("answer_short") or (ex.get("answer","")[:400].strip()))
            b = ex.get("resp_b") or (ex.get("answer_long") or (ex.get("answer","")[:800] + " Detaljer: ..." ))
            if not a or not b:
                continue
            ...
            pair = {"prompt": prompt, "A": a, "B": b, "meta": {"A": meta_a, "B": meta_b}, "win_label": win_label}
