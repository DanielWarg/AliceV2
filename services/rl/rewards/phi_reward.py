...
MISSING_POLICY = os.getenv(
    "RL_MISSING_POLICY", "skip"
).lower()  # "skip" rekommenderas (din T3)

# Säkerhets-straff: om safety_ok=False → safety=0 (hård)
SAFETY_HARD_FAIL = os.getenv("RL_SAFETY_HARD_FAIL", "true").lower() in (
    "1",
    "true",
    "yes",
    "y",
...
ALPHA = float(
    os.getenv("RL_WEIGHT_ALPHA", "2.0")
)  # precision får φ^2, latency φ^1, energy φ^0, safety φ^-1
...
def _score_precision(
    tool_success: Optional[bool], schema_ok: Optional[bool]
) -> Optional[float]:
