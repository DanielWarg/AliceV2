...
class DriftMonitor:
    ...
    def establish_baseline(self, baseline_episodes: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def detect_drift(self, current_episodes: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        p_pooled = (baseline_metrics["success_rate"]["count"] + sum(current_metrics["success_flags"])) / (n1 + n2)
        se = np.sqrt(p_pooled * (1 - p_pooled) * (1/n1 + 1/n2))
        z_stat = (current_rate - baseline_rate) / se if se > 0 else 0
        p_value = 2 * (1 - abs(z_stat)) if abs(z_stat) < 1 else 0.01  # Rough approximation
        ...
        severities = [test["severity"] for test in drift_results["drift_tests"].values()]
    ...
    def _test_categorical_drift(self, baseline_counts: Dict[str, int], current_counts: Dict[str, int]) -> Dict[str, Any]:
    ...
    def get_drift_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "baseline_established": self.baseline_stats["timestamp"] if self.baseline_stats else None,
    ...
    def _analyze_drift_trend(self) -> str:
        ...
        recent_severities = [result["overall_drift_severity"] for result in self.drift_history[-5:]]
...
if __name__ == "__main__":
    ...
    with ComponentSeed("drift_test"):
        ...
        for i in range(100):
            episode = {
                "reward_components": {"total": np.random.normal(0.5, 0.15)},  # Lower mean, higher variance
                "outcome": {
                    "latency_ms": int(np.random.normal(200, 50)),  # Higher latency
                    "success": np.random.choice([True, False], p=[0.6, 0.4])  # Lower success rate
                },
                "action": {"tool": np.random.choice(["email", "calendar"], p=[0.9, 0.1])},  # Different tool dist
