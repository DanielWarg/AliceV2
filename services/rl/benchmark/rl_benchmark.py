...
def benchmark_micro_ops() -> Dict[str, Any]:
    ...
    # Sample contexts
    contexts = [
        {"intent_conf": 0.8, "len_chars": 25, "has_question": True, 
         "cache_hint": False, "guardian_state": "NORMAL", "prev_tool_error": False},
        {"intent_conf": 0.5, "len_chars": 100, "has_question": False,
         "cache_hint": True, "guardian_state": "BROWNOUT", "prev_tool_error": True},
        {"intent_conf": 0.9, "len_chars": 10, "has_question": True,
         "cache_hint": False, "guardian_state": "EMERGENCY", "prev_tool_error": False}
    ]
...
def run_benchmark(mode: str = "all") -> Dict[str, Any]:
    ...
    with open(artefacts_dir / "qps_by_mode.csv", "w", newline="") as f:
        ...
        for mode_name, result in results["results"].items():
            ops_sec = result.get("ops_per_sec", result.get("turns_per_sec", result.get("episodes_per_sec", 0)))
...
def check_slo_gates(results: Dict[str, Any]) -> bool:
    ...
    if "micro_ops" in results["results"]:
        ...
        if micro["ops_per_sec"] < min_ops_sec:
            print(f"❌ Micro-ops too slow: {micro['ops_per_sec']:.1f} < {min_ops_sec} ops/sec")
            gates_passed = False
        else:
    ...
    if "turn_sim" in results["results"]:
        ...
        if turn["latency_p95_ms"] > max_p95_ms:
            print(f"❌ Turn-sim p95 too high: {turn['latency_p95_ms']:.1f} > {max_p95_ms}ms")
            gates_passed = False
        else:
            print(f"✅ Turn-sim p95: {turn['latency_p95_ms']:.1f}ms")
        
        if turn["success_rate"] < min_success:
            print(f"❌ Turn-sim success too low: {turn['success_rate']:.3f} < {min_success}")
            gates_passed = False
        else:
...
def main():
    ...
    parser.add_argument("--mode", choices=["all", "micro", "turn", "replay", "CI"], 
