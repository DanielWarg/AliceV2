...
def _looks_swedish(text: str) -> bool:
    return any(ch in text for ch in "åäöÅÄÖ") or re.search(SW_WORDS, text, re.I) is not None
...
def _repair_json(s: str) -> Tuple[str, bool]:
    ...
    if close_b > open_b: fixed = "{" * (close_b - open_b) + fixed
    open_a, close_a = fixed.count("["), fixed.count("]")
    if open_a > close_a: fixed += "]" * (open_a - close_a)
    if close_a > open_a: fixed = "[" * (close_a - open_a) + fixed
    changed = fixed != s
...
def _validate_markdown_rules(text: str, cfg: Dict[str, Any], reasons: list):
    ...
    if cfg.get("verifier",{}).get("require_code_block_closure", True) and not _balanced_fences(text):
...
def _validate_json_schema(text: str, reasons: list) -> bool:
    ...
    try:
        ...
        if not isinstance(obj, dict):
            reasons.append("json_not_object"); return False
        if len(obj.keys()) > 32:
            reasons.append("json_too_many_keys"); return False
        for k, v in obj.items():
            if isinstance(v, str) and len(v) > 8000:
                reasons.append("json_value_too_long"); return False
        return True
    except json.JSONDecodeError:
        # try repair once
        fixed, changed = _repair_json(raw)
        if not changed:
            reasons.append("json_parse_error"); return False
        try:
            obj = json.loads(fixed)
            return True
        except Exception:
            reasons.append("json_parse_error_after_repair"); return False
...
def verify(text: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for pat in cfg.get("labeling",{}).get("banned_patterns", []):
        if re.search(pat, text, re.I):
            ok = False; reasons.append(f"banned_pattern:{pat}")
    for word in cfg.get("verifier",{}).get("policy_forbidden", []):
        if re.search(re.escape(word), text, re.I):
            ok = False; reasons.append(f"policy_forbidden:{word}")
    ...
    return {
        "ok": ok and len([r for r in reasons if r.startswith("banned_") or r.startswith("policy_")]) == 0,
        "reasons": reasons,
