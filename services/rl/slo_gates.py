...
def run_post_training_slo_check(baseline_episodes: List[Dict], new_episodes: List[Dict]) -> bool:
    ...
    for metric, threshold in regression_thresholds.items():
        ...
        if metric == 'p95_latency_ms':
            ...
            if new_val > baseline_val + threshold:
                regressions.append(f"{metric}: {new_val:.1f} vs {baseline_val:.1f} (+{new_val-baseline_val:.1f})")
        else:
            # Lower value = worse
            if new_val < baseline_val - threshold:
                regressions.append(f"{metric}: {new_val:.3f} vs {baseline_val:.3f} ({new_val-baseline_val:+.3f})")
...
def main():
    ...
    parser.add_argument("--create-ci", action="store_true", help="Create CI check script")
