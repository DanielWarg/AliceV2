...
def create_episode_from_telemetry_event(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if output_text:
            ...
            if intent_detected == "none" or "förstår inte" in output_data.get(
                "render_instruction", {}
            ).get("content", ""):
                pass
    except Exception:
    ...
    actual_success = (
        ...
        and "förstår inte" not in output_text
...
def create_episode_from_events_old(
...
def classify_swedish_intent(text: str) -> str:
    ...
    if any(word in text_lower for word in ["klockan", "tid", "datum", "dag"]):
        ...
        word in text_lower for word in ["plus", "beräkna", "räkna", "1234", "5678"]
    ):
        return "calculation"
...
def calculate_reward(
    intent: str, latency_ms: float, success: bool, tool_calls: int
) -> float:
...
def main():
    ...
    avg_reward = (
        sum(ep.get("reward", 0) for ep in episodes) / len(episodes) if episodes else 0
    )
    ...
    print(
        f"✅ Successful episodes: {successful_episodes}/{len(episodes)} ({successful_episodes/len(episodes)*100:.1f}%)"
    )
    print(f"📈 Average reward: {avg_reward:.2f}")
    print(
        f"🎯 Fibonacci optimized: {fibonacci_optimized}/{len(episodes)} ({fibonacci_optimized/len(episodes)*100:.1f}%)"
    )
    ...
    if metadata["ready_for_training"]:
        ...
        print("Run: python3 services/toolselector/train_lora.py")
    else:
