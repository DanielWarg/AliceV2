...
def create_tool_selection_prompt(intent: str, lang: str = "sv", context: str = "") -> str:
...
def create_training_dataset(examples: List[Dict[str, str]], tokenizer, config: TrainingConfig):
...
def main():
    ...
    if len(tool_examples) < 10:
        logger.warning(f"Very few training examples ({len(tool_examples)}), results may be poor")
