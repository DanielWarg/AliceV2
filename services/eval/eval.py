import os, sys, json, time, uuid, httpx, pathlib

API = os.getenv("API_BASE","http://localhost:8000")
ART = pathlib.Path("../../data/tests"); ART.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    with RESULTS.open("a") as f:
        for sc in scenarios:
            ...
            row = {
                "v":"1","ts":time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                "id": sc["id"], "ok": ok, "lat_ms": round(dt,1), "route": route, "status": r.status_code
            }
