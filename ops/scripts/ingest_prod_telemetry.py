...
"""
import os, json, re, time, datetime, argparse, sys
from pathlib import Path

try:
    import yaml
except Exception:
    print("[ingest] pyyaml saknas; installera pyyaml i CI/venv.", file=sys.stderr); sys.exit(2)
...
def main():
    ...
    if src_type is None:
        print("[ingest] Ingen källa hittades i ops/config/telemetry_sources.yaml", file=sys.stderr)
        sys.exit(2)
    ...
    if src_type == "file":
        ...
        print("[ingest] Okänd källa", file=sys.stderr); sys.exit(2)
    ...
    for ev in reader:
        ...
        if intent not in intents_hist: intent = "qa"
    ...
    def to_list(hist): return [hist[k] for k in INTENTS]
