...
def simulate(
    new_regex_yaml="ops/suggestions/intent_regex_suggestions.yaml",
    window="data/ops/telemetry_window.json",
):
    import yaml
    ...
    if rca_path.exists():
        for line in rca_path.read_text(encoding="utf-8").splitlines():
            ...
            try:
                ex = json.loads(line)
                texts.append(ex.get("snippet_redacted", ""))
            except:
                pass
    ...
    def pad(vec):
        if len(vec) >= len(INTENTS):
            return vec[: len(INTENTS)]
    ...
    # räkna PSI
    base_cut = base[: len(INTENTS)]
    act_cut = act[: len(INTENTS)]
    sim_cut = sim_act[: len(INTENTS)]
    ...
    Path("ops/suggestions/intent_tuning.json").write_text(
