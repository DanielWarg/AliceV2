...
"""
        diff_content += yaml.dump(new_config, default_flow_style=False, allow_unicode=True)
...
def show_impact_summary(suggestions):
    ...
    if 'intent_regex' in suggestions:
        regex_count = sum(len(patterns) for patterns in suggestions['intent_regex'].values())
