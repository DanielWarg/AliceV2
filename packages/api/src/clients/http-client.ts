...
export class HTTPClient {
  ...
  async getLLMStatus(): Promise<LLMStatus> {
    return this.fetchWithRetry<LLMStatus>(
      `${this.config.baseURL}/api/v1/llm/status`
    )
  }

  // Voice Pipeline APIs
  async classifyIntent(request: NLURequest): Promise<NLUResponse> {
    return this.fetchWithRetry<NLUResponse>(
      `${this.config.baseURL}/api/nlu/classify`,
      {
        method: 'POST',
        body: JSON.stringify(request)
      }
  ...
  async generateTTS(request: TTSRequest): Promise<TTSResponse> {
    return this.fetchWithRetry<TTSResponse>(
      `${this.config.baseURL}/api/tts/`,
      {
        method: 'POST', 
        body: JSON.stringify(request)
      }
  ...
  async getGuardianStatus(): Promise<GuardianStatus> {
    return this.fetchWithRetry<GuardianStatus>(
      `${this.config.guardianURL}/health`
    )
  }

  // System Health APIs
  async getAliceHealth(): Promise<HealthStatus> {
    return this.fetchWithRetry<HealthStatus>(
