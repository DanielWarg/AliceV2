...
export function VoiceCommandInterface({ onCommand, className }: VoiceCommandInterfaceProps) {
  ...
  return (
    <div
      className={cn(
        'voice-command-interface bg-white border border-gray-200 rounded-lg p-6',
        className,
      )}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-semibold text-gray-900">🎙️ Voice Commands</h2>
      </div>
      ...
      {session && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-medium text-blue-900">Current Session: {session.intent}</h3>
              {session.processing && (
              ...
              {session.resultText && !session.processing && (
                <p className="text-sm text-green-700">✅ Complete: "{session.resultText}"</p>
              )}
            </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="mt-6 p-3 bg-gray-50 border border-gray-200 rounded-lg text-sm">
          ...
          <div className="space-y-1 text-gray-600">
            ...
            {session && (
              <p>
                Session: {session.intent} | Active: {session.active}
              </p>
            )}
          </div>
        </div>
