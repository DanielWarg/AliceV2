...
class ProductionReadinessValidator:
    ...
    def validate_t1_schema_hardening(self):
        ...
        try:
            with open("services/rl/requirements.txt") as f:
                requirements = f.read()
                locked_versions = [line for line in requirements.split('\n') if '==' in line]
                
            if len(locked_versions) >= 5:  # Major libraries locked
                self.record_pass("T1.1", "Library versions locked", f"{len(locked_versions)} packages locked")
            else:
                self.record_fail("T1.1", "Insufficient locked versions", f"Only {len(locked_versions)} packages locked")
        ...
        try:
            ...
            if result.returncode == 0 and "1.0.0" in result.stdout:
                self.record_pass("T1.2", "Schema versioning implemented", "Default version: 1.0.0")
            else:
        ...
        try:
            ...
            if "forbid" in result.stdout:
                self.record_pass("T1.3", "Strict mode enforced", "extra='forbid' configured")
            else:
        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                self.record_pass("T1.4", "Frozen seeds configured", f"Seed: {result.stdout.strip()}")
            else:
        ...
        self.run_test_validation("T1.5", "services/rl/tests/test_schema_strict_mode.py", "Schema strict mode tests")
    ...
    def validate_t2_privacy_hardening(self):
        ...
        try:
            ...
            if "True" in result.stdout:
                self.record_pass("T2.1", "PII masking functional", "Phone number detection working")
            else:
        ...
        try:
            ...
            if "OK" in result.stdout:
                self.record_pass("T2.2", "Drift monitoring available", "DriftMonitor instantiable")
            else:
        ...
        self.run_test_validation("T2.3", "services/rl/tests/test_pii_drift_coverage.py", "PII and drift tests")
    ...
    def validate_t3_reward_hardening(self):
        ...
        try:
            ...
            if "True" in result.stdout:
                self.record_pass("T3.1", "φ constant accurate", "Mathematical precision validated")
            else:
        ...
        try:
            ...
            if "OK" in result.stdout:
                self.record_pass("T3.2", "Reward validator available", "RewardValidator instantiable")
            else:
        ...
        self.run_test_validation("T3.3", "services/rl/tests/test_phi_reward_hardening.py", "φ-reward hardening tests")
    ...
    def validate_t4_persistence_hardening(self):
        ...
        try:
            ...
            if "OK" in result.stdout:
                self.record_pass("T4.1", "LinUCB router functional", "Router instantiable")
            else:
        ...
        try:
            ...
            if "OK" in result.stdout:
                self.record_pass("T4.2", "Thompson tools functional", "Tools instantiable")
            else:
        ...
        if persistence_file.exists():
            try:
                ...
                if "filelock" in content and "FileLock" in content:
                    self.record_pass("T4.3", "File locking implemented", "FileLock usage detected")
                else:
                    self.record_warning("T4.3", "File locking unclear", "FileLock usage not clearly detected")
            except Exception as e:
                self.record_fail("T4.3", "Persistence file error", str(e))
        else:
            self.record_warning("T4.3", "Persistence file not found", "May be using alternative persistence")
    ...
    def validate_t5_routing_hardening(self):
        ...
        try:
            ...
            if "FastAPI" in result.stdout:
                self.record_pass("T5.2", "FastAPI integration working", "App instance created")
            else:
        ...
        if orchestrator_file.exists():
            self.record_pass("T5.3", "Orchestrator integration exists", "RL routes file found")
        else:
            self.record_warning("T5.3", "Orchestrator routes not found", "May be integrated differently")
    ...
    def validate_t6_toolselector_hardening(self):
        ...
        if toolselector_file.exists():
            ...
            self.record_warning("T6.1", "ToolSelector v2 not found", "May be using different structure")
        
        # Check 2: GBNF schema enforcement
        try:
            result = subprocess.run([
                "find", ".", "-name", "*.py", "-exec", "grep", "-l", "gbnf\\|GBNF", "{}", ";"
            ], capture_output=True, text=True, cwd=".")
            
            if result.returncode == 0 and result.stdout.strip():
                self.record_pass("T6.2", "GBNF schema found", f"Files: {len(result.stdout.strip().split())}")
            else:
                self.record_warning("T6.2", "GBNF schema not found", "May be using different schema enforcement")
        except Exception as e:
        ...
        if makefile.exists():
            try:
                ...
                if "toolselector" in content and "rl-" in content:
                    self.record_pass("T6.3", "Makefile targets exist", "RL and ToolSelector targets found")
                else:
                    self.record_warning("T6.3", "Makefile targets incomplete", "Some targets may be missing")
            except Exception as e:
                self.record_fail("T6.3", "Makefile error", str(e))
        else:
            self.record_warning("T6.3", "Makefile not found", "Build automation may be handled differently")
    ...
    def run_test_validation(self, check_id: str, test_file: str, description: str):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                passed_line = [line for line in lines if 'passed' in line and ('failed' in line or 'error' in line or line.endswith('passed'))]
                self.record_pass(check_id, f"{description} pass", passed_line[0] if passed_line else "All tests passed")
            else:
                # Extract failure info
                error_lines = [line for line in result.stdout.split('\n') if 'FAILED' in line or 'ERROR' in line]
                self.record_fail(check_id, f"{description} fail", f"{len(error_lines)} failures")
        except Exception as e:
    ...
    def generate_go_nogo_report(self) -> Dict[str, Any]:
        ...
        critical_failures = sum(1 for r in self.results.values() if r["status"] == "FAIL")
        go_decision = critical_failures <= 1 and pass_rate >= 0.75  # Allow 1 non-critical failure
        ...
        report = {
            ...
            "go_no_go_decision": {
                ...
                "rationale": self._get_decision_rationale(go_decision, critical_failures, pass_rate),
    ...
    def _get_decision_rationale(self, go_decision: bool, critical_failures: int, pass_rate: float) -> str:
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        failed_checks = [check_id for check_id, result in self.results.items() if result["status"] == "FAIL"]
        warning_checks = [check_id for check_id, result in self.results.items() if result["status"] == "WARN"]
        
        if failed_checks:
            recommendations.append(f"CRITICAL: Fix failed checks: {', '.join(failed_checks)}")
        
        if warning_checks:
            recommendations.append(f"Address warnings: {', '.join(warning_checks)}")
        
        if self.passed > 0 and self.failed == 0:
            recommendations.append("System shows strong production readiness - consider deployment")
