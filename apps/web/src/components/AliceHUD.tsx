...
export function AliceHUD({ config, className }: AliceHUDProps) {
  ...
  return (
    <div className={`alice-hud min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white ${className}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <Card className="bg-gray-800/50 border-gray-700">
            ...
            <CardContent className="p-6">
              <div className="h-32">
                <AudioVisualizer 
                  isActive={isVoiceListening && voiceFeaturesEnabled} 
                />
              </div>
            </CardContent>
          </Card>
              ...
              {guardianStatus ? (
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span>State:</span>
                    <span className={
                      guardianStatus.status === 'normal' ? 'text-green-400' :
                      ['degraded', 'emergency'].includes(guardianStatus.status) ? 'text-red-400' :
                      'text-yellow-400'
                    }>
