...
const Pane = ({
  ...
  <div
    className={`relative rounded-2xl border border-cyan-500/20 bg-cyan-950/20 p-4 shadow-[0_0_60px_-20px_rgba(34,211,238,.5)] ${className || ''}`}
  >
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setW({
        ...
        desc: ['Sunny', 'Cloudy', 'Partly cloudy', 'Light rain'][Math.floor(Math.random() * 4)],
      });
    }, 5000);
    ...
    if (!isListening) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          setMessages(prev => [
            ...prev,
            { role: 'assistant', content: 'Hej! Jag mår bra, tack! Hur kan jag hjälpa dig?' },
          ]);
          setIsTyping(false);
        }, 1000);
      }, 3000);
          ...
          <Pane title="System">
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              ...
              <RingGauge
                size={80}
                value={mem}
                icon={<IconDrive className="h-3 w-3" />}
                showValue={false}
              />
              <RingGauge
                size={80}
                value={net}
                icon={<IconGauge className="h-3 w-3" />}
                showValue={false}
              />
            </div>
          </Pane>
              ...
              <div className="h-full overflow-y-auto p-3 space-y-1 scrollbar-thin scrollbar-thumb-cyan-500/30 scrollbar-track-transparent">
                ...
                <div className="text-cyan-400">
                  ...
                  {guardianStatus === 'NORMAL'
                    ? '5/5'
                    : guardianStatus === 'BROWNOUT'
                      ? '3/5'
                      : '1/5'}
                </div>
                <div className="text-blue-400">[DEBUG] Last health check: {now}</div>
                <div className="text-blue-400">
                  [DEBUG] Memory usage: {systemDiagnostics?.ram_pct || 0}%
                </div>
                <div className="text-blue-400">
                  [DEBUG] CPU usage: {systemDiagnostics?.cpu_pct || 0}%
                </div>
                {systemDiagnostics?.temp_c && (
                  <div className="text-blue-400">
                    [DEBUG] CPU temperature: {systemDiagnostics.temp_c}°C
                  </div>
                )}
                {systemDiagnostics?.battery_pct && (
                  <div className="text-blue-400">
                    [DEBUG] Battery level: {systemDiagnostics.battery_pct}%
                  </div>
                )}
                <div className="text-blue-400">
                  [DEBUG] Uptime: {Math.floor((systemDiagnostics?.uptime_s || 0) / 60)}m{' '}
                  {Math.floor((systemDiagnostics?.uptime_s || 0) % 60)}s
                </div>
      ...
      <footer className="pointer-events-none absolute inset-x-0 bottom-0 mx-auto max-w-7xl px-6 pb-8">
        <div className="grid grid-cols-5 gap-4 opacity-80">
          {['SYS', 'NET', 'AUX', 'NAV', 'CTRL'].map((t, i) => (
            <div
              key={i}
              className="relative h-14 overflow-hidden rounded-xl border border-cyan-500/20 bg-cyan-900/10"
            >
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-cyan-200/90 text-lg uppercase tracking-widest">
                    Inställningar
                  </h2>
                </div>
                <button
