...
class EvalHarness:
    ...
    def load_scenarios(
        self, regression_dir: str = "eval/regression"
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if isinstance(actual_tools, str):
                    ...
                    actual_tools = [
                        tool for tool in actual_tools if tool and tool != "none"
                    ]
                else:
                    actual_tools = []
            ...
            if expected_tools:
                tool_precision = len(
                    expected_tools.intersection(actual_tools_set)
                ) / len(expected_tools)
            else:
    ...
    def calculate_metrics(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        def calculate_level_metrics(
            level_results: List[Dict[str, Any]], level_name: str
        ) -> Dict[str, Any]:
            ...
            schema_ok_rates = [
                r.get("schema_ok_first", False) for r in successful_results
            ]
            ...
            return {
                f"{level_name.lower()}.schema_ok_first": (
                    sum(schema_ok_rates) / len(schema_ok_rates)
                    if schema_ok_rates
                    else 0.0
                ),
                f"{level_name.lower()}.latency_p50_ms": (
                    statistics.median(latencies) if latencies else 0
                ),
                f"{level_name.lower()}.latency_p95_ms": (
                    statistics.quantiles(latencies, n=20)[18]
                    if len(latencies) >= 20
                    else max(latencies) if latencies else 0
                ),
                f"{level_name.lower()}.tool_precision": (
                    sum(tool_precisions) / len(tool_precisions)
                    if tool_precisions
                    else 0.0
                ),
                f"{level_name.lower()}.success_rate": len(successful_results)
                / len(level_results),
            }
        ...
        all_tool_precisions = [
            r.get("tool_precision", 0.0) for r in results if r.get("success", False)
        ]
        ...
        metrics = {
            ...
            "overall": {
                ...
                "tool_precision": (
                    sum(all_tool_precisions) / len(all_tool_precisions)
                    if all_tool_precisions
                    else 0.0
                ),
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        print("🎯 Quality Gates:")
        print(
            f"  EASY+MEDIUM schema_ok ≥ 95%: {'✅' if (metrics['easy.schema_ok_first'] + metrics['medium.schema_ok_first']) / 2 >= 0.95 else '❌'}"
        )
        print(
            f"  Tool precision ≥ 85%: {'✅' if metrics['tool_precision'] >= 0.85 else '❌'}"
        )
        print(
            f"  Latency P95 ≤ 900ms: {'✅' if metrics['latency_p95_ms'] <= 900 else '❌'}"
        )
        print("=" * 60)
...
async def main():
    ...
    parser.add_argument(
        "--output", default="eval_runs/eval_report.json", help="Output file for report"
    )
    ...
    try:
        ...
        quality_gates_passed = (
            (
                metrics["easy"]["easy.schema_ok_first"]
                + metrics["medium"]["medium.schema_ok_first"]
            )
            / 2
            >= 0.95
            and metrics["overall"]["tool_precision"] >= 0.85
            and metrics["overall"]["latency_p95_ms"] <= 900
        )
