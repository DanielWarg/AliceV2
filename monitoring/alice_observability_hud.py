...
def create_energy_chart(events_data: List[Dict[str, Any]]) -> go.Figure:
    ...
    df = pd.DataFrame(
        [
            {"timestamp": event.get("ts", ""), "energy_wh": event.get("energy_wh", 0)}
            for event in events_data
...
def main():
    ...
    st.markdown(
        "Real-time monitoring av RAM peak, energy, tool errors och SLO compliance"
    )
    ...
    events_file = st.sidebar.text_input(
        "Events JSONL file", value="data/telemetry/events_2025-08-31.jsonl"
    )

    guardian_file = st.sidebar.text_input(
        "Guardian JSONL file", value="data/telemetry/guardian_2025-08-31.jsonl"
    )
    ...
    with col1:
        ...
        st.metric(label="Guardian Status", value=f"{status_icon} {state}", delta=None)
    ...
    with col3:
        if events_data:
            ...
            st.metric(label="Latest RAM Peak", value=f"{proc_mb:.1f} MB", delta=None)
        else:
    ...
    with col4:
        if events_data:
            ...
            st.metric(label="Latest Energy", value=f"{energy_wh:.4f} Wh", delta=None)
        else:
    ...
    if events_data:
        ...
        for event in recent_events:
            with st.expander(
                f"Event {event.get('ts', 'unknown')} - {event.get('route', 'unknown')}"
            ):
                st.json(event)
    else:
