...
def create_guardian_timeline_chart(df: pd.DataFrame) -> go.Figure:
    ...
    for state in df["state"].unique():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(color=COLORS.get(state, COLORS["unknown"]), size=8),
    ...
    fig.add_hline(
        y=80, line_dash="dash", line_color="orange", annotation_text="RAM Soft (80%)"
    )
    fig.add_hline(
        y=92, line_dash="dash", line_color="red", annotation_text="RAM Hard (92%)"
    )
...
def create_latency_chart(status_data: Dict[str, Any]) -> go.Figure:
    ...
    if route_names:
        # P95 bars
        fig.add_trace(
            go.Bar(x=route_names, y=p95_values, name="P95", marker_color="lightcoral"),
            row=1,
            col=1,
        )
    ...
    fig.add_trace(
        go.Indicator(
            ...
            gauge={
                "axis": {"range": [0, max(100, total_requests * 1.2)]},
                "bar": {"color": "lightblue"},
...
def create_metrics_cards():
    ...
    with col4:
        ...
        if status.get("ok"):
            ...
            budget_color = (
                "red" if r5xx_rate > 1 else "orange" if r5xx_rate > 0.5 else "green"
            )
...
def render_security_panel():
    ...
    with col2:
        ...
        if den:
            df_den = pd.DataFrame(
                {"reason": list(den.keys()), "count": list(den.values())}
            )
